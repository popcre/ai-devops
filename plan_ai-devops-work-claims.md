...
4. After launch, every `acquire` success/refusal and advisory finding writes a bounded public audit marker `<!-- ai-work-claim-event:v1 -->` plus JSON fields `event`, `task`, `ref_key`, `owner_hash`, `timestamp`, and `reason_code` to the task issue. The follow-up command is bounded and time-scoped, never a full-history scan: `gh api --method GET repos/popcre/ai-devops/issues/comments -f per_page=100 -f since="$SINCE" -f sort=created -f direction=desc | jq -c '.[] | select(.body | contains("<!-- ai-work-claim-event:v1 -->"))' > claim-events.jsonl`, where `$SINCE` is the start of the reporting window and the command stops after at most five pages. This is reporting only. **Audit history is never read to decide whether a task is claimed** - availability comes from the exact claim ref alone, whose read cost does not grow with history. An implementation that scans issue comments, commit history, or closed pull requests to answer "is this claimed?" is rejected on review.
...
Every claim command runs under a strict request budget declared in `config/work-claim-policy.json`: at most five GitHub requests to read availability and at most ten for a full `acquire`, counted in the tool and enforced before each call. Before taking the claim ref - the shared mutex - `acquire` reads `gh api rate_limit` and refuses with a specific non-polling message when the remaining core allowance is under 200 or a secondary/abuse limit is signalled. Refusing before the ref is taken is required: exhausting quota mid-acquisition is what strands a claim nobody can release. Batch and cache task-issue and pull-request reads within a single command invocation; never re-read the same object twice. Exceeding the budget is a defect, not a retry condition.
...
**Verification gate:** a fixture carrying 10,000 historical claim events and closed task issues completes `list`, `status`, and a full `acquire` in fewer than 20 GitHub requests, counted by a recording stub, and the count does not rise when the fixture doubles; a simulated exhausted or secondary-limited quota refuses before the ref is created and leaves no claim behind; every failure path releases the claim ref or reports `audit-pending` with the ref still owned - none strands it silently; wrong-token operations fail; stale claims block new acquire; force-with-lease rejects stale heartbeat/release/path-extension attempts; interruption preserves the ref; issue closure permits only the same owner; offline/403 conditions never block Git commit/push and claim commands produce one non-polling error; manual reconciliation rejects changed object/open PR and produces complete audit evidence after exact authorization.
