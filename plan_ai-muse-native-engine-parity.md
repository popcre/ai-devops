...
Handoffs retired at Phase D close (2026-09-24): `HANDOFF.d/2026-09-17T1416Z-edge-dev-claude-muse-native-parity-plan.md`, `HANDOFF.d/2026-09-23T2313Z-edge-dev-claude-phase-d-muse-native.md`.
...
| C1 | `AI_MUSE_REASONING_EFFORT` support + metadata record + tests | ✅ done | PR #678 (merge `57170b62`): `muse_code_reasoning_effort` in `cmd_new`/`cmd_ask`/`run_turn` validates the tier against the catalog row's `reasoning_effort_variants` when readable (non-empty all-string array containing the tier) else the fixed CLI list `none|minimal|low|medium|high|xhigh|max|ultra`, refusing `start_failed` before any provider contact; `--reasoning-effort` appended only when the env is non-empty (unset/empty args byte-identical); effective tier recorded in `retained_turn.reasoning_effort`; OpenCode ignores the variable (documented + `test-ai-muse.sh` case). `bash tests/test-ai-muse-code.sh` 90 PASS/0 FAIL and `python tests/fixtures/muse-code/usage_cases.py -q` 27 OK at merged head `926da622`. |
| C2 | `delete` also removes the date-bucketed durable log + tests | ✅ done | PR #678 (merge `57170b62`): `muse_code_delete_session` removes `<XDG_DATA_HOME>/muse/sessions/*/*/*/<sid>` plus the `.msp-view-v1` projection under one preflight — verified physical root, no linked ancestors/date-buckets/UUID, every parent readable+searchable, UUID strictly a directory (the projection parent gained the same bar after round-1 Grok REJECT, fix `566846bd`) — then deletes each store from its verified parent and requires positive confirmation, naming the surviving store otherwise. `tests/fixtures/muse-code/delete_cases.sh` 11 passed; full suites green (`test-ai-muse-code.sh` 90/0, `test-ai-muse.sh` 168/0, opencode contract pass); independent Grok 4.6 APPROVE at exact head `926da622` (round 1 REJECT fixed, rounds 2-5 APPROVE); installed-route doctor PASS. |
| D1 | Live qualification turns on the muse-code engine | ✅ done | Muse session `review-20260923T114151Z-2415847-31868` on the native engine (`e0a41811-2a2d-4b7f-b692-0e955885dc94`), model `muse-spark-1.3-contributor`. New turn + `ask` follow-up; `VERDICT: NO FINDINGS` under `require_verdict: 1`. Non-null usage with catalog-priced estimates. Cited on [#542 comment](https://github.com/popcre/ai-devops/issues/542#issuecomment-5794520101). |
| D2 | shared-db `REVIEWERS` evidence update (scripts/docs PR) | ✅ done | `popcre/shared-db` PR [#3433](https://github.com/popcre/shared-db/pull/3433) MERGED 2026-09-23T22:33:31Z. `muse-spark-1.3-contributor` `readsRepositoryVerified` date `2026-09-23` with native-engine evidence on `origin/main`. Non-orchestrator scripts work — no db-work issue. |
| D3 | Default flip to `muse-code` + docs + test expectation flip | ✅ done | `popcre/ai-devops` PR [#725](https://github.com/popcre/ai-devops/pull/725) MERGED as `ee697287` (2026-09-24T01:48:26Z). `bin/ai-muse` default `ENGINE="${AI_MUSE_ENGINE:-muse-code}"`; OpenCode suite pins `AI_MUSE_ENGINE=opencode`. CI windows-offline-section 2/6 green (`test-ai-muse.sh`, `test-ai-muse-code.sh`). Independent Grok 4.6 APPROVE at `cf9aabea`. Rollback: `AI_MUSE_ENGINE=opencode`. |
| D4 | Post-flip governed review through shared-db verified | ✅ done | Post-flip governed Muse review of `popcre/shared-db` PR [#3441](https://github.com/popcre/shared-db/pull/3441) / issue #3440 at head `35c7141c89b9bff8d230dd489d87e65f247b529c`. Assignment: Muse replacement sequence 3647 (lease `refs/db-review-active-v2/muse-spark-1.3-contributor/3440-3441-35c7141c…`). Doctor PASS (preflight without `--skip-doctor`; local doctor all PASS on muse-code). Verdict with coverage: `APPROVE` in [findings comment](https://github.com/popcre/shared-db/pull/3441#issuecomment-5806436232); durable `refs/db-review-verdict-replacements/3440-3441-35c7141c…-3640` @ `6ef38f1a3dd4687289360c0465ca6723ab435b80`. Usage: runtime `1.3.0-R3233.1`, `core-complete`, non-null counters, `catalog_cost_estimate` 0.01086077 USD. Summary on [#542](https://github.com/popcre/ai-devops/issues/542#issuecomment-5806490980). |
...
All `bin/ai-muse` line numbers are as of `origin/main` `57170b62` (2026-09-23,
after Phase C landed in PR #678).
...
doctor muse-code catalog checks in `cmd_doctor` (~:707). Phase C added
`muse_code_reasoning_effort` after the catalog helpers (~:232), the effective
tier in `retain_turn` metadata, and rewrote `muse_code_delete_session`
(~:652) into the dual-store preflight + verified-parent removal.
...
- Handoff: `HANDOFF.d/2026-09-17T1416Z-edge-dev-claude-muse-native-parity-plan.md` (retired in #749 after Phases D1-D4 landed).
