...
- `repolock_path()` stores the in-flight lock beneath the Grok state directory. `lock_acquire()` uses atomic directory creation, records the owner process, and reclaims a lock only when its recorded owner is dead.
...
7. **Install one lifecycle trap with truthful interruption reporting.** Refactor duplicated trap strings into one idempotent cleanup function that releases the exact session/repository locks and temporary files it owns. On `INT`/`TERM`, terminate/wait for the local child using existing ownership, then emit exactly one warning: local wrapper stopped; provider cancellation confirmed / not available / not confirmed; remote work may still run and bill when unconfirmed. Never let cleanup change a completed result into failure or print the warning on ordinary exit. **You'll know it worked when:** signal fixtures release only their own locks, preserve other sessions, return nonzero, and contain the required warning without claiming unproven cancellation.
