...
| 1.1 | Replace all seven poll loops in `tests/test-ai-grok-review.sh` | ✅ done 2026-08-28 | Adopted from PR [#123](https://github.com/popcre/ai-devops/pull/123) (`050ad1e`), `9aec837` closing its last two silent waits, and `fe7c0606` converting the three drift-exposed waits from a frozen deadline to progress-sensitive stall detection. Unit-proven by `tests/test-lib-test-timing.sh` (10 checks, 10 passed), which shows a slow-but-advancing fixture is no longer failed and a stalled one still is. **Do not cite the earlier single 191/0 run as proof — it was one quiet run and was not representative.** The mechanism is done; the flake-rate measurement is 1.2 and is still open. |
| 1.2 | Prove it: 10 consecutive green reviewer runs on Windows CI | 🟥 **not satisfied — no clean series exists yet** | The frozen-baseline cause named here is now fixed in `fe7c0606` (see 1.1), so the change this row demanded has been made and **Decision B was honoured — no multiplier, timeout, retry, or quarantine was added**. What is still missing is the measurement. Two local series were abandoned: the first contaminated by a self-inflicted concurrent suite, the second killed by its own cleanup, which also cancelled CI job `98712820009`. Both are recorded in `tests/verification/reviewer-flake-89/2026-08-28-local-series-abandoned.md`. Windows verification now runs on the self-hosted runner added in `2f0c081f` (`docs/self-hosted-windows-runner.md`); that machine hosts either the checks or a local series, never both. This row closes only on ten consecutive green runs on an idle machine. Issue [#89](https://github.com/popcre/ai-devops/issues/89) stays open. |
