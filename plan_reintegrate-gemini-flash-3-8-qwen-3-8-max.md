...
| Shared quarantine/preflight | GEMINI AVAILABLE; QWEN AVAILABLE ON EDGE-DEV 2026-09-16 | Gemini's EDGE-DEV record still binds exact wrapper, Antigravity, and `gemini-3.8-flash-high`. Qwen is not retired. The 2026-09-04 quarantine was lifted on EDGE-DEV after official standalone 0.23.0 was restored and `ai-review-preflight qualify qwen` succeeded. An npm `qwen` on PATH is not the approved runtime. |
| Qualification and reviewer rotation | GEMINI ACTIVE; QWEN ACTIVE ON EDGE-DEV 2026-09-16 | Both may receive governed read-only assignments on a host whose current live qualification still matches. Re-check `ai-review-preflight status qwen` before assuming Qwen is usable; registry membership is not health. |
