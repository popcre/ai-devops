...
check "source head movement during a governed paid turn refuses publication" "test '$gov_rc' -ne 0 && test ! -s '$TMP/gov-moved.out' && grep -q 'review source changed' '$TMP/gov-moved.err'"
...
check "default model is pinned to DeepSeek-V4.1-Flash" "DEEPSEEK_STUB_REQUEST='$TMP/pinned-model-request.json' run send pinned-default >/dev/null && jq -e '.model==\"DeepSeek-V4.1-Flash\"' '$TMP/pinned-model-request.json'"
