...
require "$HANDOVER" "gh pr checks <n> --repo popcre/shared-db --required --watch --interval 30"
require "$HANDOVER" "until gh pr checks <n> --repo popcre/shared-db --required >/dev/null 2>&1 || [ \$? -eq 8 ]; do sleep 10; done"
require "$HANDOVER" "gh pr merge <n> --repo popcre/shared-db --squash"
...
# The fast route must never recommend an admin merge for shared-db, under
# either the current name or the pre-transfer alias.
if grep -Eq 'gh pr merge[^`]*(popcre|u2giants)/shared-db[^`]*--admin' "$HANDOVER"; then
