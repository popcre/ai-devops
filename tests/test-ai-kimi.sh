...
UNSAFE_DIR="$(cd "$UNSAFE_DIR" && pwd -P)"; UNSAFE_JSON_DIR="$(cygpath -m "$UNSAFE_DIR")"
jq --arg dir "$UNSAFE_JSON_DIR" '.name="recover-unsafe"|.job_id="recover-unsafe"|.phase="failed"|.terminal_reason="readonly-tree-changed"|.artifact_kind=null|.artifact_sha256=null|.artifact_paths.stream=($dir+"/stream.jsonl")|.artifact_paths.log=($dir+"/worker.log")|.artifact_paths.canonical=($dir+"/review-recovery.md")|.artifact_paths.repository=null' "$DURABLE_META" > "$UNSAFE_DIR/job.json"
