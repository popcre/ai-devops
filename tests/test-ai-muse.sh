...
check 'Windows setup passes the Muse script as a direct Bash argument' "grep -Fq '& \$gitBash \$museSetup' '$ROOT/bin/setup-machine.ps1' && ! grep -Fq '& \$gitBash -lc \"'\''\$museSetup'\''\"' '$ROOT/bin/setup-machine.ps1'"
check 'model comes from tool-versions config' "grep -q 'MODEL=\"meta-model-api/\$MUSE_MODEL_ID\"' '$SCRIPT' && jq -e '.models.muse==\"muse-spark-1.3\"' '$ROOT/config/tool-versions.json' >/dev/null"
...
check 'config pins exact protected provider and model' "jq -e '.model==\"meta-model-api/muse-spark-1.3\" and .small_model==.model and .share==\"disabled\" and .autoupdate==false and .provider[\"meta-model-api\"].options.baseURL==\"https://api.meta.ai/v1\" and .provider[\"meta-model-api\"].options.apiKey==\"{env:MODEL_API_KEY}\"' '$ROOT/config/opencode-muse/opencode.json'"
...
check 'live doctor requires an exact bounded provider verdict' "grep -q \"VERDICT: APPROVE\" '$SCRIPT' && grep -q 'AI_MUSE_DOCTOR_TIMEOUT' '$SCRIPT'"
...
poll_worker_until "$PREP_PID" "$(budget 10 10)" 'preparation published its marker' '[ -s "$PREP_MARKER" ]' || true
...
cp "$META" "$TMP/debate-meta-current.json"
jq '.model="meta-model-api/muse-spark-1.2-contributor"' "$META" > "$TMP/debate-meta-old.json" && mv "$TMP/debate-meta-old.json" "$META"
rm -f "$TMP/old-model-ask-called"
check 'old-model sessions cannot cross into Spark 1.3' "cd '$REPO' && ! eval \"$ENV MUSE_STUB_TOUCH='$TMP/old-model-ask-called' '$SCRIPT' ask debate --prompt invalid\"; test ! -e '$TMP/old-model-ask-called'; jq -e '.status==\"active\" and .model==\"meta-model-api/muse-spark-1.2-contributor\"' '$META'"
mv "$TMP/debate-meta-current.json" "$META"
...
poll_worker_until "$SLOW_PID" "$(budget 30 30)" 'the slow provider turn started' '[ -s "$SLOW_PROVIDER_PID" ]' || true
poll_worker_until "$SLOW_PID" "$(budget 5 5)" 'durable in-progress state was published' 'm=$(find "$TMP/state" -name "codex--visible-slow.json" -type f -print -quit); [ -n "$m" ] && jq -e ".status==\"turn_in_progress\" and (.worker_pid|type)==\"number\" and (.last_heartbeat_at|length)>0" "$m"' || true
SLOW_META="$(find "$TMP/state" -name 'codex--visible-slow.json' -type f -print -quit)"
