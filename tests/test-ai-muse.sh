...
jq '.model="meta-model-api/muse-spark-1.3-contributor"' "$META" > "$TMP/debate-meta-old.json" && mv "$TMP/debate-meta-old.json" "$META"
rm -f "$TMP/old-model-ask-called"
check 'old-model sessions cannot cross models' "cd '$REPO' && ! eval \"$ENV MUSE_STUB_TOUCH='$TMP/old-model-ask-called' '$SCRIPT' ask debate --prompt invalid\"; test ! -e '$TMP/old-model-ask-called'; jq -e '.status==\"active\" and .model==\"meta-model-api/muse-spark-1.3-contributor\"' '$META'"
