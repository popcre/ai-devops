...
check 'exact Contributor model is pinned' "grep -q 'meta-model-api/muse-spark-1.3-contributor' '$SCRIPT'"
...
check 'session-name path components are bounded (path-length class)' "grep -q 'short_name \"muse-\$CALLER-\$1\" 64' '$SCRIPT' && grep -q 'short_name \"\$CALLER--\$2\" 96' '$SCRIPT'"
