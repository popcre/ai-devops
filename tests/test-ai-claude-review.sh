...
TESTS_OUT="$(cd "$R" && "$SCRIPT" diff-review --tests 'test -f "new file.txt"')"
check "focused tests are accepted and included before Claude review" "test -s '$TESTS_OUT' && grep -A1 '^## Verdict' '$TESTS_OUT' | tail -1 | grep -qx APPROVE"
