...
check 'every non-preferred Windows job runs on Blacksmith' "[ \"\$(grep -cE '^[[:space:]]*runs-on:[[:space:]]*blacksmith-4vcpu-windows-2025[[:space:]]*\$' '$workflow')\" -eq 3 ]"
...
[ "$blacksmith_pool" -eq 3 ] || {
  printf 'FAIL: the three non-preferred Windows verify jobs must run on Blacksmith
' >&2
  exit 1
}
