...
- `windows_sensitive_bash` contains 19 conservative platform-sensitive suites.
...
- `windows_offline_bash` owns the other 17 Windows-sensitive suites. Their
  measured baseline sum was 3,265 seconds. The remaining 48 Bash suites are
...
| Event | Before | After |
|---|---|---|
| pull request | all Bash plus all PowerShell in `windows-offline`; Codex/Grok repeated in reviewer lane | 17 offline Windows-sensitive Bash plus all PowerShell; Codex/Grok once in reviewer lane |
