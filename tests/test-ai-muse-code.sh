...
CATALOG_ROW='{"model_id":"muse-spark-1.3","provider_id":"meta","visibility":"visible","context_limit":1007997,"output_limit":128000,"is_current":true,"cost":{"input":"0.10","output":"0.20","cached":"0.002","currency":"USD"}}'
...
check 'doctor fails when the catalog has no row for the configured model' "cd '$REPO' && out=\$(eval \"$ENV '$SCRIPT' doctor\"); rc=\$?; [ \$rc -ne 0 ] && printf '%s\\n' \"\$out\" | grep -q 'FAIL  model catalog has a row for muse-spark-1.3'"
write_catalog '{"model_id":"muse-spark-1.3","provider_id":"meta","visibility":"hidden","context_limit":1007997,"output_limit":128000,"is_current":true,"cost":{"input":"0.10","output":"0.20","cached":"0.002","currency":"USD"}}'
check 'doctor fails when the catalog row is not visible' "cd '$REPO' && out=\$(eval \"$ENV '$SCRIPT' doctor\"); rc=\$?; [ \$rc -ne 0 ] && printf '%s\\n' \"\$out\" | grep -q 'FAIL  model row for muse-spark-1.3 is visible'"
write_catalog '{"model_id":"muse-spark-1.3","provider_id":"meta","visibility":"visible","context_limit":1007997,"output_limit":128000,"is_current":false,"cost":{"input":"0.10","output":"0.20","cached":"0.002","currency":"USD"}}'
...
printf '{"rows":{"nested":{"model_id":"muse-spark-1.3","visibility":"visible","context_limit":1,"output_limit":1,"is_current":true}}}' > "$CATALOG/malformed__rows-object.json"
...
write_catalog '{"model_id":"muse-spark-1.3","provider_id":"someone-else","visibility":"visible","context_limit":1007997,"output_limit":128000,"is_current":true,"cost":{"input":"0.10","output":"0.20","cached":"0.002","currency":"USD"}}'
...
printf '{"profile_id":"tbh","provider_id":"meta","rows":[%s],"schema_version":1,"source":"manual"}' '{"model_id":"muse-spark-1.3","provider_id":"meta","visibility":"visible","context_limit":1007997,"output_limit":128000,"is_current":true,"cost":{"input":"0.10","output":"0.20","cached":"0.002","currency":"USD"}}' > "$CATALOG/manual__file.json"
...
check 'launch is read-only: no write, shell, web, personal context or prompts' "for f in exec --json --disable-write --disable-shell --disable-web-tools --no-foreign-personal-context --user-input-auto-resolve; do grep -qx -- \"\$f\" '$TMP/provider-args' || exit 1; done && grep -qx 'muse-spark-1.3' '$TMP/provider-args'"
