...
mkdir -p "$TMP/countbin"; for tool in sha256sum jq; do real="$(command -v "$tool")"; printf '#!/usr/bin/env bash\nprintf "%%s\n" "$*" >> %q\nexec %q "$@"\n' "$TMP/count-$tool" "$real" > "$TMP/countbin/$tool"; chmod +x "$TMP/countbin/$tool"; done
check 'doctor hashes the Muse binary at most twice and launches jq at most four times' "cd '$REPO' && rm -f '$TMP/count-sha256sum' '$TMP/count-jq' && eval \"$ENV PATH='$TMP/countbin:$TMP/bin:$PATH' '$SCRIPT' doctor\" >/dev/null && [ \$(grep -c 'muse' '$TMP/count-sha256sum') -le 2 ] && [ \$(wc -l < '$TMP/count-jq') -le 4 ]"
