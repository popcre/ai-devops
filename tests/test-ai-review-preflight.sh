...
check "Codex and DeepSeek mirror shared-db RETIRED_REVIEWERS" "jq -e '.providers.codex.registry_state==\"absent\" and .providers.deepseek.registry_state==\"absent\"' '$REAL_REGISTRY'"
check "the shipped rotation pool is exactly Muse, Grok, Qwen, Gemini plus the Claude gate" "jq -e '[.providers|to_entries[]|select(.value.registry_state==\"registered\")|.key]|sort==[\"claude\",\"gemini\",\"grok\",\"muse\",\"qwen\"]' '$REAL_REGISTRY'"
