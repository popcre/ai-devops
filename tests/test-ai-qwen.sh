...
PREFLIGHT_CALLS_BEFORE="$(wc -l < "$TMP/argv.txt")"; DOCTOR_PREFLIGHT="$(run doctor --preflight)"
check 'preflight doctor proves identity launcher and governed auth without starting Qwen' "test \"\$(printf '%s\\n' \"\$DOCTOR_PREFLIGHT\" | wc -l)\" -eq 1 && printf '%s\\n' \"\$DOCTOR_PREFLIGHT\" | grep -Eq '^PREFLIGHT runtime_sha256=[0-9a-f]{64} preloader_sha256=[0-9a-f]{64} launcher=present auth=governed$' && test '$PREFLIGHT_CALLS_BEFORE' = \"\$(wc -l < '$TMP/argv.txt')\""
check 'preflight doctor fails closed when the approved launcher is missing' "! AI_QWEN_BIN='$TMP/missing-qwen' run doctor --preflight"
cp "$AI_QWEN_HOME/settings.json" "$TMP/settings-good.json"
jq '.security.auth.selectedType="invalid"' "$TMP/settings-good.json" > "$AI_QWEN_HOME/settings.json"
check 'preflight doctor fails closed when governed authentication is invalid' "! run doctor --preflight"
cp "$TMP/settings-good.json" "$AI_QWEN_HOME/settings.json"
check 'doctor rejects conflicting modes' "! run doctor --identity --live && ! run doctor --identity --preflight && ! run doctor --live --preflight"
