...
export AI_REVIEW_LIFECYCLE_DIR="$TMP/state/review-lifecycle"
...
printf '{"repo":"%s","head":"%s","run_id":"run-x","session_id":"x","caller":"codex","prompt":"private code","total_tokens":42,"credential":"do-not-copy","report_path":"%s","log_path":"%s"}\n' "$REPO_CANON" "$HEAD" "$REPO_CANON/.ai/reviews/grok-x-example.md" "$STATE_PROVIDER_CANON/sessions/x/stream.jsonl" > "$TMP/state/grok/sessions/x/session.json"
...
printf '{"provider":"grok","repo":"%s","head":"%s","run_id":"run-x","session_id":"x","caller":"codex","elapsed_seconds":901,"failure_class":"no-verdict"}\n' "$REPO_CANON" "$HEAD" > "$AI_REVIEW_SCOREBOARD_FILE"
DIAGNOSTIC="$($ROOT/bin/ai-review-lifecycle observe --provider grok --repo "$TMP/repo" --run-id run-x --session-id x --caller codex --phase terminal --observation provider-failed --elapsed 901 --failure-class no-verdict)"
jq '.events[0].secret="do-not-copy"' "$DIAGNOSTIC" > "$DIAGNOSTIC.tmp" && mv "$DIAGNOSTIC.tmp" "$DIAGNOSTIC"
...
output="$($SCRIPT record --provider grok --summary 'Reviewer returned no verdict.' --details-file "$TMP/details.txt" --command 'ai-grok-review ask test' --repo "$TMP/repo" --error-file "$TMP/error.log" --run-id run-x --session-id x --caller codex)"
...
check "issue JSON reports only evidence that was actually captured" "jq -e '.evidence.complete_matching_review_reports==\"review-reports/\" and .evidence.complete_matching_review_reports_captured==1 and .evidence.recent_provider_logs_captured==1 and .evidence.latest_scoreboard_entry_captured==1' '$report/issue.json'"
...
check "exact diagnostic envelope is captured" "jq -e '.run_id==\"run-x\" and .events[0].last_observation_type==\"provider-failed\"' '$report/reviewer-diagnostic.redacted.json'"
check "diagnostic secret fields are removed" "! jq -e '.events[0]|has(\"secret\")' '$report/reviewer-diagnostic.redacted.json'"
...
check "missing evidence paths are null with zero captured counts" "jq -e '.evidence.complete_matching_review_reports==null and .evidence.complete_matching_review_reports_captured==0 and .evidence.recent_provider_logs==null and .evidence.recent_provider_logs_captured==0 and .evidence.latest_scoreboard_entry==null and .evidence.latest_scoreboard_entry_captured==0' '$uncorrelated_report/issue.json'"
mkdir -p "$TMP/state/grok/sessions/empty"
printf '{"repo":"%s","head":"%s","session_id":"empty","caller":"codex"}\n' "$REPO_CANON" "$HEAD" > "$TMP/state/grok/sessions/empty/session.json"
empty_capture="$($SCRIPT record --provider grok --summary 'Exact metadata, no owned evidence.' --repo "$TMP/repo" --session-id empty --caller codex)"
empty_capture_id="$(printf '%s\n' "$empty_capture" | sed -n 's/^ai-reviewer-issue: recorded //p')"
empty_capture_report="$AI_REVIEWER_ISSUE_DIR/$empty_capture_id"
check "exact metadata with zero captures is labelled truthfully" "grep -q 'Exact reviewer metadata matched, but no owned review report, provider log, or scoreboard entry was captured' '$empty_capture_report/missing-evidence.txt'"
check "exact metadata with zero captures advertises no evidence paths" "jq -e '.evidence.complete_matching_review_reports==null and .evidence.recent_provider_logs==null and .evidence.latest_scoreboard_entry==null' '$empty_capture_report/issue.json'"
...
check "ambiguous metadata cannot capture a scoreboard row" "test ! -e '$ambiguous_report/latest-scoreboard-entry.json' && jq -e '.evidence.latest_scoreboard_entry==null and .evidence.latest_scoreboard_entry_captured==0' '$ambiguous_report/issue.json'"
