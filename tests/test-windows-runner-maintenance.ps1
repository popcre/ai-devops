...
$launcherPath = Join-Path $repo 'bin\launch-worker.bat'
...
try {
  ...
  function Register-MaintenanceTask { param($OperatorSid) $script:mockTask = [ordered]@{ execute='C:\Windows\System32\cmd.exe'; arguments='/d /c "C:\Program Files\ai-devops\windows-runner-maintenance\launch-worker.bat"'; action_count=1; user_id=$OperatorSid; logon_type='S4U'; run_level='Highest'; trigger_count=0; multiple_instances='IgnoreNew'; sddl="D:P(A;;FA;;;SY)(A;;FA;;;BA)(A;;GRGX;;;$OperatorSid)"; state='Ready' } }
  ...
  Case 'Launcher_IsAnEnvironmentAllowlist' { Assert-Contains $installer '/d /c "C:\Program Files\ai-devops\windows-runner-maintenance\launch-worker.bat"'; Assert-Contains $worker '/d /c "C:\Program Files\ai-devops\windows-runner-maintenance\launch-worker.bat"'; Assert-Contains $launcher 'for /f "delims==" %%v in (''set ^| C:\Windows\System32\findstr.exe /r /c:"^[A-Za-z0-9_-]*="'') do set "%%v="'; Assert-Contains $launcher 'set "SystemRoot=C:\Windows"'; Assert-True (-not $launcher.Contains('DOTNET_STARTUP_HOOKS')) 'launcher is still a denylist, not an allowlist' }
  ...
  Case 'Payload_LauncherIsHashPinned' { Assert-True ($policy.payload_hashes.'launch-worker.bat' -ceq (Get-FileHash -Algorithm SHA256 $launcherPath).Hash.ToLowerInvariant()) 'policy launcher hash is stale' }
  ...
  Case 'RecoverPartial_RefusesComOverridesBeforeFirstTaskCmdlet' { Assert-True ($installer.IndexOf('Assert-NoPerUserComOverride', $installer.IndexOf('function Recover-MaintenanceInstallation')) -lt $installer.IndexOf('Get-ScheduledTask -TaskPath $script:TaskFolder -TaskName $script:TaskName -ErrorAction SilentlyContinue', $installer.IndexOf('function Recover-MaintenanceInstallation'))) 'recovery touches task COM before the override refusal' }
  Case 'RecoverPartial_AssertsRuntimeRootBeforeOwnedNameDeletes' { $r = $installer.IndexOf('function Recover-MaintenanceInstallation'); $rootAssert = $installer.IndexOf('Assert-NoReparsePoint -LiteralPath $script:RuntimeRoot -AllowMissing', $r); $loop = $installer.IndexOf('foreach ($name in $script:OwnedRuntimeNames) {', $r); Assert-True ($rootAssert -gt 0 -and $loop -gt $rootAssert) 'runtime root not asserted before the owned-name deletes' }
  Case 'RecoverPartial_AssertsPayloadTreeBeforeBackupCopiesIt' { $r = $installer.IndexOf('function Recover-MaintenanceInstallation'); $treeAssert = $installer.IndexOf('Assert-NoReparsePoint -LiteralPath $script:PayloadRoot', $r); $backup = $installer.IndexOf('Backup-MaintenanceInstallation -Destination $RecoveryPath', $r); Assert-True ($treeAssert -gt 0 -and $backup -gt $treeAssert) 'payload tree not asserted before the backup copies it' }
  Case 'RecoverPartial_AssertsRuntimeParentToo' { $r = $installer.IndexOf('function Recover-MaintenanceInstallation'); Assert-Contains $installer.Substring($r, [Math]::Min(4000, $installer.Length - $r)) 'Assert-NoReparsePoint -LiteralPath $runtimeParent -AllowMissing' }
  Case 'RecoverPartial_BackupPathBindsWithInstall' { Assert-True ($installer -match "ParameterSetName='Remove'\)\]\[Parameter\(ParameterSetName='Install'\)\]") 'BackupPath cannot be supplied to -RecoverPartial -Install' }
