...
| 12. Land: tests green, merge, install, verify | 🔄 in progress | 2026-09-17 | New suites registered in `config/ci-suite-manifest.json`; affected suites re-run green locally (parity, blocker-watch, cmd-launchers, machine-tools, completion-check-hook, adopt-globals, install-manifest, tool-version-pins). Merge + edge-dev install + installed-state verification: this PR and its post-merge follow-up. |
