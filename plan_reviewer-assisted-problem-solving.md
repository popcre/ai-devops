# IMPLEMENTATION PLAN — reviewer-assisted problem solving (2026-08-31; corrected 2026-09-01)

Linked handoff: [`HANDOFF.d/2026-09-01T1059Z-edge-dev-codex-reviewer-problem-solving-corrections.md`](HANDOFF.d/2026-09-01T1059Z-edge-dev-codex-reviewer-problem-solving-corrections.md)
...
**Independent plan review:** Kimi K3 reviewed merged head `5dd46b5976f4018c15ebf1b94c7730fef56f0c05` in read-only job `bab0e5ba6e64-codex-reviewer-problem-solving-plan-k3` and returned `READY WITH NON-BLOCKING IMPROVEMENTS`. This corrected version incorporates all seven findings plus its debate-bound observation. That review is design input, not exact-head approval of future implementation.
...
This is a workflow capability request, not a reproducible software crash. Neither global contains a reviewer-assisted stuck rule; only the Codex template currently has a broad delegation-cost section (`templates/system/AGENTS-global-codex.md:121-139`), while both templates share the capability-preservation guidance under “When something goes wrong” (`templates/system/AGENTS-global-codex.md:53-63`; `templates/system/CLAUDE-global.md:53-63`). Provider skill descriptions mostly trigger when Albert names a provider (`skills/shared/ask-glm/SKILL.md:3`, `skills/shared/deepseek-second-opinion/SKILL.md:3`, `skills/shared/grok-cli/SKILL.md:3`, and `skills/shared/qwen-code/SKILL.md:3`).
...
- The new shared skill is owned by issue #198 under Albert's repository. Its retirement path is removal of the skill and both global routes in one reviewed commit; the installer then moves managed installed copies to recoverable quarantine.
...
- `docs/architecture.md:72-79` now contains the planning pointer for issue #198. Persistent debate continuity and the rule that current artifacts must be re-read begin at `docs/architecture.md:81`; the pointer is not an implemented route.
...
8. **Trigger evaluation and live behavior are different evidence.** The committed skill-trigger harness measures whether a prompt that explicitly narrates prior failed attempts selects the skill from its description. It does not recreate a multi-turn session or prove proactive escalation from accumulated history. Live installed-client probes are the behavior gate.
...
11. The route's one-initial-turn plus one-focused-follow-up bound is stricter than any provider skill's ordinary debate allowance and supersedes that allowance only while `reviewer-problem-solving` owns the escalation.
12. Issue #198 owns the shared skill. Retirement removes the skill and both global routes together in one reviewed commit; managed installed copies move to recoverable quarantine through the existing installer.
...
2. **Location of the global paragraph:** place the matched rule immediately after the shared “When something goes wrong” section in both templates (`templates/system/*global*.md:53-63`), while preserving client-specific surrounding text and context budgets.
...
6. **Bound the loop.** Default to one turn plus one focused follow-up. This route-specific bound supersedes a provider skill's more permissive ordinary debate limit while this skill owns the escalation. Permit one alternate provider only for unusable output, failed preflight/provider health, or a material unresolved disagreement. Never rotate reviewers to obtain a preferred answer.
...
Do not copy the full procedure into globals; the shared skill owns it. Keep GPT-5.6 reasoning restrictions unchanged. Add one stable `PARITY_RULES` entry in `tools/context-audit/context-audit.py` keyed to the route name `reviewer-problem-solving`, and make `tests/test-reviewer-problem-solving-skill.sh` compare the threshold/exclusion semantics in both templates. The audit's regex parity is not semantic comparison, so the structural test owns wording alignment. Do not weaken budgets or unrelated markers.

**Dependencies:** Step 1 establishes the canonical behavior.
**Verification gate:** `python tools/context-audit/context-audit.py --root . --claude-home <test-or-real-home> --codex-home <test-or-real-home> --json .ai/context-current.json --strict` reports no strict safety/parity failure, and the structural test proves both templates carry the same threshold and exclusions. Record and justify any context-budget warning as measured growth from this named capability; never raise a budget or weaken the skill description merely to silence it.
...
- `AGENTS.md`: replace the existing planning-only issue #198 router row with a durable route for a session struggling with a diagnosis or repeated repair, pointing to the shared skill and durable architecture after completion.
...
- `docs/codex-skills-usage-guide.md`: add the shared skill to the Codex-facing shared-skill map and state that the same proactive route is installed for Codex.
- `docs/architecture.md`: replace the existing `Planned reviewer-assisted problem solving` pointer at lines 72-79 with the durable implemented architecture; retain the plan link as a completed decision record only after STATUS proves completion.
...
6. Run the Claude description-trigger eval with `tools/skill-trigger-eval/skill-trigger-eval.py` and the Codex description-trigger eval with `tools/skill-trigger-eval/codex-trigger-eval.py`, at the required repeated count on Windows and Linux per `docs/skill-trigger-eval.md`. Record raw JSON artifacts under the established verification tree. Inspect the transcript/artifact for every positive miss before editing the description because a miss may be an eval-mechanism limitation. Tune only the new skill description; no positive miss or negative false trigger may be dismissed without evidence. Label these artifacts `description discrimination`, never `live proactive behavior`.

**Dependencies:** Steps 1-3. Test authoring can proceed alongside documentation after the skill contract stabilizes.
**Verification gate:** all structural and offline suites pass; Claude and Codex description-discrimination evals meet policy on Windows and Linux with zero protected near-miss false triggers; every miss has transcript evidence; and the STATUS table links the exact artifacts without claiming they prove accumulated-session behavior.
...
4. From a clean current `main` on this Windows machine, run `bin/install-ai-devops-windows.ps1` as the supported Windows skill installer, then use the documented global-adoption path so both existing machine sections are backed up, re-appended, and byte-compared. On Ubuntu use `bin/ai-install-skills` and `bin/ai-adopt-globals` through the documented user/path. Do not substitute the Windows Bash fallback for the preferred native installer in acceptance evidence.
5. Verify on disk—not from source alone—that:
   ...
   Run a committed, fixture-driven probe harness three times per client. Each positive fixture must present the desired behavior, same symptom, and two distinct failed evidence-based attempts (or one explicit unresolved contradiction) in the conversation input; each negative fixture presents only the excluded condition. The harness scores whether the client routes to `reviewer-problem-solving` without actually dispatching an external reviewer. Then run one controlled end-to-end positive per installed client that permits exactly one healthy read-only reviewer call. Save the exact sanitized fixture, client, source SHA, run result, and reviewer-call count. These installed-client probes—not the description eval—are the proactive-behavior evidence.
...
- The Claude description-trigger eval at the policy-required repeated count on Windows and Linux
- The Codex description-trigger eval through `tools/skill-trigger-eval/codex-trigger-eval.py`, separately labeled from the Claude eval
- A committed installed-client probe harness with three repeated positive/negative fixture runs per client, plus one controlled end-to-end positive per client
...
- Context-budget warnings are measurements, not strict failures. Record and justify attributable growth; never increase the ratchet or degrade trigger text to force zero warnings.
...
- Planning checkout: `C:\tmp\ai-devops-plan-198`; the correction branch was `codex/reviewer-problem-solving-plan-corrections-198`. A successor may use a new clean checkout and must not assume this temporary path still exists.
...
- [ ] Claude and Codex positive/negative description-discrimination evals are committed and pass policy repeatedly on Windows and Linux without being mislabeled as live behavior.
...
- [ ] Repeated installed-client fixture probes and one controlled end-to-end positive per client prove genuine stuck escalation and decline first-failure, owner-decision, and external-wait near misses.
...
- **Context growth:** the universal rule and skill manifest may cross a warning budget. Preserve the trigger and safety contract, record the attributable bytes, and follow the ratchet policy; a warning alone is not permission to raise the limit or cut required behavior.
...
1. **Could a brand-new AI session execute this plan without asking Albert anything? Yes.** §§2-6 define the product, trigger, current planning pointers, existing capabilities, and missing route; §§8-9 lock the behavior and name exact files, client-specific evals, installation paths, sequence, dependencies, and verification gates; §§11-12 define operating constraints and access.
2. **Does the plan preserve all relevant background, nuance, and rejected approaches? Yes.** §§6-8 capture the root cause, Kimi's trigger-eval and debate-bound concerns, diagnostic-versus-approval distinction, provider-selection limit, privacy boundary, threshold reasoning, and every rejected shortcut discussed or discovered.
...
- [x] Kimi's seven findings and debate-bound observation are all corrected: eval scope/Codex runner, citations/anchor, budget semantics, parity ownership, stale planning pointers, Codex docs/Windows installer, artifact governance, and route-specific turn precedence.
- [x] The plan and this correction session's handoff link directly to each other.
