...
Tailscale address is `100.110.219.31`. The Windows user is `ahazan2` and the
...
- On 2026-08-10 at 11:29 UTC, Git for Windows SSH to
  `100.110.219.31:22` timed out after eight seconds. No remote command ran and
...
2. A direct Git for Windows SSH call with `BatchMode=yes` and an eight-second
   connection timeout returned `connect to host 100.110.219.31 port 22:
...
1. Turn on `916` and confirm it has joined Tailscale. You will know this worked
   when `100.110.219.31:22` accepts an SSH connection and `hostname` identifies
   the `916` computer.
...
- Target: `916-alien`, Tailscale `100.110.219.31`, Windows user `ahazan2`.
