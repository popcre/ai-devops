...
3. Resume PR #560 in its existing worktree: rebase onto current `origin/main`, correct `plan_windows-runner-maintenance-elevation.md` Step 5 and restart point, rerun focused tests, obtain one exact-head security APPROVE, and use `bin/ai-pr-wait 560`. Merge only if required checks and review qualify the final head. Do not install on either Windows host in that session.
