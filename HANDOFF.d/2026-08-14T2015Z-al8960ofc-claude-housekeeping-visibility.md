...
1. **Approve starting the plan, and say where to start.** The plan is written and
   self-audited but nothing has been built. Recommendation: approve steps 1-3
   (Phase A) immediately — they are two-line documentation fixes plus one rule
   change, and steps 1 and 2 correct instructions that are actively misleading
   sessions today.
...
3. **Delete this file** once the plan is finished, or once the plan file itself
   is deleted. It has no other purpose.
...
- **No cap on the number of `HANDOFF.d/` files.** Owner ruling 2026-08-13, quoted
  at `skills/shared/handoff-writer/SKILL.md:167-171`. Count STALE files instead.
...
**Written but NOT yet committed:** `plan_repo-housekeeping-visibility.md` and
this file.
...
**The root cause is the absence of a machine-checkable "done" signal.** The
...
make done-ness a one-command lookup is optional here — the heading at
`handoff-writer/SKILL.md:60` reads "REQUIRED **where the repo enforces it**", and
this repo enforces nothing. So the later session guesses. Guessing is what left
27 finished files behind in `shared-db` (issue #658, cleaned 2026-08-13).

That finding is why the plan makes the contract block mandatory (step 3) *before*
building the reporting tool (step 4). Without it the tool can only print a list of
filenames and hand Albert back the archaeology.
...
**Cheap provable wins already visible:** two branches sit on the identical commit
...
1. **Commit and push `plan_repo-housekeeping-visibility.md` and this file.**
...
CI and deploys. Section 3 gives the pushed SHA, exactly which files are written
but uncommitted, the untracked paths belonging to other sessions, and the
...
deliberate plan records). Section 4 carries five dead ends, three of them my own:
...
him?** Yes, checked by walking sections 1 through 9. Three items: approve the
start and where to begin, decide whether to open a GitHub issue, and delete this
file when the work is done. Everything else is finished analysis or an
instruction to a future session.

Self-audit passed on 2026-08-14.
