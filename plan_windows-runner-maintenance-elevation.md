...
| 6. Install and prove `edge-dev-win` | 🟨 in progress | 2026-09-18 | Owner authorized the install in chat; deploy gate passed; first elevated attempt (approved dispatch at d32bc84d) failed at Set-MaintenanceTaskAcl with 0x8007007B - the raw Schedule.Service COM interface rejects a trailing folder separator. Fix landed as 6f102cf3 (exact-head APPROVE cd028153; collateral findings from the wide packet filed as issue #601). Recovery-and-install one-shot is staged on the host; the retry dispatch approval is blocked only by reviewer availability (claude repeatedly quarantined provider-unhealthy; codex usage-capped until 2026-09-19 ~04:13Z). Do NOT run the staged desktop launcher before that approval exists. The failed attempt left an inert partial state (registered no-trigger task, installed trees) that the staged recovery step removes first. |
