...
| 5. Obtain exact-head independent security approval and land | 🟨 in progress | 2026-09-17 | Implementation PR #560, rebased onto current `origin/main`. Reviews 1-3 (`b0a5a7a1`, `8a72a9fc`, `b1b6007e`) each REJECTed with findings repaired on the same branch. Review 4 (`2305f6a2`) REJECT: the launcher's naive clear loop was itself exploitable (COMSPEC child hijack; variable-name injection through cmd re-parsing), install adopted foreign-owned pre-existing state, removal lacked immediate pre-delete re-verification, and four lows — repaired: the launcher pins COMSPEC first and clears only findstr-filtered injection-proof names (verified by local execution), the temp root is administrators-only, host names come from the .NET API, foreign-owned pre-existing paths are refused at install, every elevated delete re-verifies immediately, the recovery bundle is pinned admin-only, the versioned ProgID shadow is refused, and the installer self-hardens against hostile shell variables. A fifth exact-head review of the repaired head is required before any merge; protected gate unchanged. |
