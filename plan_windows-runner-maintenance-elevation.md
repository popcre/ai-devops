...
| 5. Obtain exact-head independent security approval and land | 🟨 in progress | 2026-09-17 | Implementation PR #560, rebased onto current `origin/main`. Reviews 1-5 (`b0a5a7a1`, `8a72a9fc`, `b1b6007e`, `2305f6a2`, `9a30ad19`) each REJECTed with findings repaired on the same branch. Review 6 (`58eeff61`) REJECT with no High findings: the recovery-backup prefix allowlist was defeated by an ancestor junction and a pre-existing destination was re-ACLed wholesale; lows on installer-side parent verification, runtime-parent check ordering, environment-derived operator identity and abandoned-mutex handling — repaired: every existing ancestor of the backup destination is junction-checked (and ownership-checked under ProgramData), the destination must be new or empty-and-admin-owned, `-Verify` checks the evidence parent with the same contract as the worker, the runtime parent ownership is verified before any tree is created beneath it, the installer derives the machine name from the .NET API, and an abandoned mutex is acquired rather than crashing. Review 7 (`ed7fa089`) returned explicit APPROVE with hardening-only findings and ruled RemoteSigned acceptable. CI then exposed a repo convention (every bin/*.cmd must be a shim for an extensionless sibling), so the launcher was renamed launch-worker.bat (byte-identical content); the rename changes the head and therefore requires one more exact-head review before merge. |
