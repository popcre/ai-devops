...
| 5. Obtain exact-head independent security approval and land | 🟨 in progress | 2026-09-17 | Implementation PR #560, rebased onto current `origin/main`. Review 1 (`b0a5a7a1`) REJECT: session hijack surface, fail-open audit/ledger, unprotected evidence file, lows — repaired. Review 2 (`8a72a9fc`) REJECT: ProgID/.NET startup-hook channels, evidence tmp sibling and parent, group-granted ACL rights, single-action drift, canonical-GUID replay — repaired. Review 3 (`b1b6007e`) REJECT: launcher denylist missed the CoreCLR profiler channel (fixed with a hash-pinned allowlist `launch-worker.cmd` that clears the whole inherited environment), operator Modify on the evidence file was itself a defect (dropped; elevated writers use Administrators), requests-junction cleanup deletion, ledger poisoning, installer COM override, lows — repaired on the same branch. A fourth exact-head review of the repaired head is required before any merge; protected gate unchanged. |
