...
| 5. Obtain exact-head independent security approval and land | 🟨 in progress | 2026-09-17 | Implementation PR #560, rebased onto current `origin/main`. First exact-head review at `b0a5a7a1` returned REJECT (session hijack surface via module/COM resolution, fail-open audit/ledger, unprotected evidence file, plus four low and one informational finding); all findings were repaired on the same branch with new hostile tests. The second exact-head review at `8a72a9fc` also returned REJECT: ProgID-resolution and .NET startup-hook hijack channels, the evidence tmp sibling and parent directory, group-granted ACL rights, single-action drift, and canonical-GUID replay were each named and each is repaired on the same branch. A third exact-head review of the repaired head is required before any merge; protected gate unchanged. |
...
- Review-driven tightening of the task action (second exact-head review, 2026-09-17, head `8a72a9fc`): the action is launched through `C:\Windows\System32\cmd.exe /d` so that no .NET host starts inside the operator-controlled inherited S4U environment; the launcher strips the .NET startup-hook and root-override variables before the same machine-wide `pwsh.exe` command runs. This tightens the locked action without broadening the operation or the privilege boundary.
