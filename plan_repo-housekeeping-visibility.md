...
**Albert must be able to see, in one command and one screen, everything left
lying around in this repo that nobody is working on any more — and see who left
it — without asking an AI session to go digging.**
...
1. One command prints every stale artifact class in this repo, in plain English,
   and names the session that owns each item.
2. That report runs on its own schedule on every machine, so it does not depend
   on any AI session remembering to run it.
3. "Is this handoff finished?" is answerable by a machine, in one lookup, instead
   of by reading a nine-section essay and guessing.
...
**Rules that exist and are correct**
...
**Finding 3 (the root cause) — there is no machine-checkable "done" signal.**
...
contract block that would make done-ness a one-command lookup is optional here
(`handoff-writer/SKILL.md:60`, "REQUIRED **where the repo enforces it**"). This
repo enforces nothing. So the later session guesses, and guessing is what left
27 files behind in `shared-db`.
...
With it, "stale" becomes a fact a script can establish. That is why step 3 comes
before step 4.
...
|---|---|---|
| **Machine-local** | worktrees, agent job records under `~/.local/state/ai-devops/{glm,kimi,qwen}`, scratch clones | Cleanup affects only this machine. Nothing to commit |
| **Shared via git** | `HANDOFF.d/` files, `plan_*.md`, branches, `refs/codex/*` | Deleting one is a **git commit** in a repo several sessions write to concurrently. It can collide mid-commit, and on another machine the file returns at the next pull |
...
feels toothless: stop. That is the rejected approach.** Report-only that prints
the exact command for a human is the whole design.
...
Twenty concurrent workstreams legitimately means twenty files. Count STALE files
instead; the target for those is zero.
...
the one that cannot prove done-ness (finding 3). Steps 1-3 still tighten the
...
- **D2 — No cap on `HANDOFF.d/` file count. Count stale files; target zero.**
...
- **D5 — The contract block becomes mandatory in this repo before the tool is
  built.** It is what makes "stale" machine-checkable. Basis: finding 3.
...
- **O1 — Exact output format.** A single flat list versus one block per artifact
  class. Criterion: Albert must understand every line without asking. Follow the
  `ai-workspace-status` house style. Recommendation: one labelled block per class,
  each ending with the literal command to fix it.
...
**Change:** `skills/claude/wrap-up/SKILL.md`, the retention bullet at lines
...
> **Do not count files. Count STALE files — those whose `issue:` is already
> closed — and report those by name with their `owner:`. The target for stale is
> zero; there is no limit on the total.** (Owner ruling 2026-08-13; see
...
```bash
grep -rn "more than 5\|more than five" skills/ templates/ docs/ AGENTS.md
```

returns **no** match that instructs a warning on handoff-file count. (A quote of
the superseded rule inside `handoff-writer`'s "this is superseded" paragraph is
allowed and expected; a live instruction is not.)
...
2. If the `916` rollout is still genuinely blocked (last measured: host
   `100.110.219.31:22` refused a connection on 2026-08-12), rewrite the sentence
...
**Intent:** after this step, any reader — human or script — can answer "is this
handoff finished?" with one `gh issue view <n>`.

**Depends on:** nothing, but step 4's stale detection depends on it.

**Verification gate:** write a throwaway handoff file yourself following the
updated instructions, and confirm `gh issue view <the number you used>` returns a
state. Then delete the throwaway. Also confirm the rule survives a fresh read:
`grep -n "contract block" AGENTS.md skills/shared/handoff-writer/SKILL.md templates/system/handoff-standard.md`
returns a hit in all three.
...
```bash
#!/usr/bin/env bash
# ai-housekeeping — read-only inventory of stale artifacts in this repo.
#
#   ai-housekeeping            # print the report
#   ai-housekeeping --log      # also append the report to the housekeeping log
#   ai-housekeeping --quiet    # print nothing when there is nothing stale
#
# READ-ONLY BY DESIGN: it never deletes, commits, checks out, or prunes
# anything. Every finding ends with the exact command a human can run.
# There is deliberately no --apply. See plan_repo-housekeeping-visibility.md.
set -uo pipefail
```

**Behavior, section by section. Print each as its own labelled block (O1).**
...
each, report: path, branch, whether the working tree is dirty, whether its branch
has an open or merged PR (via `gh pr list --head <branch> --state all`), and
whether any live process holds it. Classify:

- `LIVE` — dirty, or has an open PR, or a process holds it.
- `MERGED` — clean and `gh` proves a merged PR for its branch.
- `UNKNOWN` — anything else, including all rows when `gh` is unavailable.

Print for each `MERGED` row the literal remedy, and say who runs it:

```
  remedy: use the cleanup-worktree skill on <path>   (a human or an informed session, never this tool)
```

**C. Branches — GIT-SHARED.** For every local branch except `main`: its short
SHA, its PR state from `gh`, and two cheap provable extras that need no network —
...
- `STALE` — `issue:` names a number and `gh issue view <n>` reports `CLOSED`.
  **This is the number the owner cares about; target zero.**
- `OPEN` — issue still open.
- `UNPROVABLE` — `issue: none`.
- `UNATTRIBUTED` — no contract block at all (a rule violation after step 3).

Print `owner:` on every line so the mess is attributable (finding 3, and the
...
**G. Odd refs — GIT-SHARED.** Count `refs/codex/turn-diffs/checkpoints/*`. Report
...
`3 stale handoffs, 6 merged worktrees, 15 unowned refs. Nothing was changed.`
Exit `0` when it ran correctly, whatever it found. Exit non-zero only when the
tool itself could not run (not in a repo). A stale artifact is not an error.
...
3. `contains no destructive command` — grep the script source and fail on
   `rm `, `git commit`, `git checkout`, `git branch -d`, `git branch -D`,
   `git worktree remove`, `git worktree prune`, `git reset`, `git clean`.
...
5. `reports UNKNOWN when gh is unavailable` — run with a `PATH` that has no `gh`;
   ...
   code is still `0`.
6. `classifies a closed-issue handoff as STALE` — fixture handoff file with a
   contract block, with `gh` stubbed by a fake on `PATH` returning `CLOSED`.
...
9. `exits 0 when it finds stale artifacts` — a finding is not a failure.
...
**Verification gate:** `bash tests/test-ai-housekeeping.sh` prints all nine cases
...
3. **`AGENTS.md`** — two edits: add `ai-housekeeping` to the installed-commands
   ...
   46) reading roughly: *"Find stale branches, worktrees, handoff files, or agent
...
4. **`docs/`** — add a short `docs/housekeeping.md` (there is none today) that
   explains in plain English what each artifact class is, what stale means for
   it, and who is allowed to clean it. Link it from the new `AGENTS.md` row.
...
2. Decide the cadence. `ai-memory-sync` fires every 30 minutes; a housekeeping
   ...
   separate daily Scheduled Task. **Recommendation: (b), a separate task** — it
...
4. Surface it where Albert will actually see it: add one line to the closing
   ...
   run `ai-housekeeping` and report stale counts in plain English.
...
(`Start-ScheduledTask -TaskName <name>` in PowerShell), then confirm
...
2. For every `MERGED` worktree, use the `cleanup-worktree` skill — **one at a
   time, proving each one holds no unique uncommitted work first** (finding 5).
   Do not batch them.
3. For every `STALE` handoff, follow the successor rule's three conditions at
...
gone and the `UNKNOWN`/`retained` items unchanged. `git worktree list` and
...
| Test | Where | Asserts |
|---|---|---|
| New: `tests/test-ai-housekeeping.sh` | new file | The nine cases named in step 5 |
...
Two of the nine cases exist purely to stop a future session undoing a decision:
case 3/4 encode D1 and D3, and case 8 encodes R4. Do not weaken them.
...
- [ ] Steps 1-8 all `✅ done` in the STATUS table, each with a real artifact in
      the evidence cell.
- [ ] `bash bin/ai-housekeeping` runs clean from `C:\repos\ai-devops` and prints
      all eight blocks.
- [ ] `bash tests/test-ai-housekeeping.sh` — all nine cases pass, `0 failed`.
...
| The report marks a live worktree `MERGED` and someone deletes real work | D1 means the tool never deletes. Step 8 requires proving each worktree individually. Finding 5 is the recorded near-miss |
...
2. **Does the `916` machine rollout still matter?** Step 2 forces an answer.
   Last measured 2026-08-12: `100.110.219.31:22` refused a connection.
...
**Checklist re-graded after that fix: all items pass.** Self-audit passed on
2026-08-14.
