...
2026-08-13), `hetz` (2026-08-13) and `albt16` (2026-08-14). `916-alien` is
excluded: powered off, Albert's call, 2026-08-13. **Read the step-8 drift block
and the step-9 row first** — in particular that `installed source drift` is 2 on
a machine carrying a machine section and 0 on one without (`hetz`), so the
acceptance gate must read "only the globals may differ", never a fixed number.
Machine sections are no longer re-appended by hand: use `bin/ai-adopt-globals`.
