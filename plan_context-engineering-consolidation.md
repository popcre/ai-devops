...
6. **SAFETY FINDING, unresolved: `codex-shared-db-change` scores 8/10.** It misses
   **its own verbatim trigger phrase**, "make db changes the proper way", and it
   misses Rule 0 schema inspection ("review the schema — what columns exist…").
   False positives are 0/10, so the skill is not over-firing; it is under-firing.
   Re-tested with `--project C:\repos\popdam3`, a real app repo on the shared
   database, and both still miss — **this is not a "wrong repo" artifact.** The
   consequence is that a Codex session could make a shared-database change without
   opening the discipline that keeps app repos from authoring their own
   migrations. Nobody has fixed this. The likely cause is a ~1,000-character
   description that buries its trigger phrases behind a long parenthetical, but
   **that is a hypothesis, not a measurement**. Any fix must keep the skill's
   NAME (both globals point at it), keep global-versus-skill-description overlap
   at 0, and be justified by a re-score, not by reading better.
