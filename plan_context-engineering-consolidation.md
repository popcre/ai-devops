...
| 10. Measure results and close the workstream | ✅ done | 2026-08-14 | **Final budgets set from measurement, not from a percentage guess** — each one is the size that was live on all three rolled-out machines when every probe passed, edited in all three places (`budgets.json`, `DEFAULT_BUDGETS`, the `docs/context-engineering.md` table): always-loaded **33,311 → 26,946 (−19.1%)**, startup-routed **50,729 → 37,088 (−26.9%)**, Claude manifest 21,521 → 22,777 (+5.8%), Codex manifest 14,015 → 14,847 (+5.9%), duplicate paragraph groups 12 → 3. Budget warnings now **0**; `--strict` exits 0. **The always-loaded budget was RAISED 24,713 → 26,946, the one sanctioned raise in this repo's history**, because commit `df59ffa` added Albert's own shared-db STRUCTURE-not-data ruling to both globals; it is recorded as a decision, not absorbed silently, and the new target is 24,500 rather than the unreachable 23,318. **The manifests grew and that is reported, not hidden.** Results, the rollout table, and the open questions are written up under "Results (step 10)" in `docs/context-engineering.md`. **Gap recorded honestly: the five matched before/after task evaluations (test 16) were NOT run** — the pre-trim globals are gone from every machine, so no honest "before" arm exists, and restoring one would risk the rollout for a number nobody would act on; the evidence is the six committed probes (`tools/context-probes/`, pass on two machines) plus the trigger scores. **Deliberately left as they are:** mixed CRLF/LF (normalizing rewrites every machine for zero behavioural gain — compare with `tr -d '\r'`), and `installed source drift`, whose correct value is 2 with a machine section and 0 without, so the gate reads "only the globals may differ". **This machine's dead `Z:` trap trimmed to a pointer** (568 → 263 bytes of always-loaded text) now that `machine-atlas.md` holds the full story. **The step-8 conclusion that an always-loaded rule naming a skill SUBSTITUTES for it is RETRACTED, by measurement:** the same skill, same eval set, same `--runs 3`, same global naming it outright, measured on the same day, scores **10/10 should-fire on `hetz` and 5/10 on `al8960ofc`, 0/10 false positives on both**. That also means the step-8 §0 question "should rule 9a's procedure move into the global?" is answered **no** — nothing is suppressing the skill. **A second finding falls out and it undercuts earlier work:** Windows measured 2/10, then 1/10, then 5/10 for this skill across three days **with no text change at all**, a swing larger than any description edit yet measured, so step 7a's two "failed" rewrites were never actually shown to fail. A single `--runs 3` score is an observation, not a verdict. The surviving open question is the platform gap (Linux 10/10 vs Windows 5/10), most likely the neutral eval project having no Synology MCP wired up; safety is unaffected because the 25-second limit lives in the global and probes confirm it on both platforms |
| 11. Re-audit and tighten the startup instructions | ✅ done | 2026-08-21 | Albert authorized a second audit of both clients and this repo. Always-loaded text is now **13,871 bytes** and startup-routed entry text is **11,987 bytes**; the budgets are ratcheted to those measured values with targets 12,500 and 11,000. The strict audit reports zero missing safety markers, parity mismatches, broken links, global/skill overlaps, or installed-source drift. Detailed Synology, shared-database, production-incident, reviewer, deployment, DesignFlow, and handoff procedures now load only when their task triggers them. |

**Fresh-session start:** this plan is closed; there is no continuation step.
The latest 2026-08-21 measurements and routing decisions are in
[`docs/context-engineering.md`](docs/context-engineering.md). Machine sections
must still be preserved through `bin/ai-adopt-globals`, never re-appended by
hand.
...
**Newest handoff: none — this workstream has no open handoff.** All ten steps
closed on 2026-08-14 and every step handoff was retired with its work; git
history keeps the text. The durable record is this plan's STATUS table plus
"Results (step 10)" in [`docs/context-engineering.md`](docs/context-engineering.md).
(When you delete a handoff, fix every link that names it — this line was itself
a broken pointer for exactly that reason.)
...
divergence-allowlist entry. `tests/test-context-audit.ps1` removes each of the seven
locked safety categories independently and requires a plain-English reason naming
that category, and proves the other six are unaffected. Parity now covers twelve
rules that must appear in both globals, including destructive actions recoverable
and system binaries protected, plus an allowlist for genuinely client-only
...
- [x] Always-loaded global plus repo-start context is materially smaller —
      13,871 and 11,987 bytes at the 2026-08-21 re-audit.
