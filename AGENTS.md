...
| Reviewer, provider, CI, runner, throughput, active plan, or known incident | [`docs/task-router.md`](docs/task-router.md); for Blacksmith Windows throughput read [`plan_blacksmith-windows-throughput.md`](plan_blacksmith-windows-throughput.md) STATUS first | Resolve current status; preserve safety behavior |
