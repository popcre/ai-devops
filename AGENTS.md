...
| Fix delegated-review latency, no-verdict runs, provider rotation, exact-head invalidation, or non-database work consuming shared-db agents | **[`plan_reviewer-system-repair.md`](plan_reviewer-system-repair.md) FIRST — its STATUS table and its DRIFT RECORDED block**, then [`fix_reviewer_system.md`](fix_reviewer_system.md) for the original analysis, then the affected Grok/GLM/Kimi wrapper instructions above | Steps 1-3 (sealed evidence packets) shipped 2026-08-18; steps 4-9 are open. **The speed premise is NOT proven**: a live A/B on 2026-08-18 cut wall time 18% and cost 10% but left the turn count unchanged at 8, so do not lower any turn or time ceiling before re-measuring. Preserve fail-closed safety; the repair is never broader permissions or weaker approval gates |
