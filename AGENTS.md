...
| Build or continue the Muse Spark 1.2 protected review runner | **[`plan_muse-opencode-harness.md`](plan_muse-opencode-harness.md) STATUS first**, then `docs/muse-opencode.md`, `tests/test-ai-muse.sh`, and `skills/shared/ask-muse/SKILL.md` | Issue #40 owns this work. Muse is a direct protected review runner, not a live service: it uses a disposable self-contained copy and must never substitute standard Muse or fall back to another model. GLM remains its separate persistent service. |
...
| Fix delegated-review latency, no-verdict runs, provider health, failure handling, exact-head invalidation, or non-database work consuming shared-db agents | **[`plan_reviewer-system-repair.md`](plan_reviewer-system-repair.md) FIRST — its STATUS table and newest DRIFT RECORDED block**, then [`fix_reviewer_system.md`](fix_reviewer_system.md), [`docs/reviewer-issues.md`](docs/reviewer-issues.md), `bin/ai-review-preflight`, `bin/ai-review-scoreboard`, `bin/ai-reviewer-issue`, and the affected wrapper instructions above | The evidence packet shipped. Five additional Grok 4.6 reviews returned 5/5 usable verdicts in 6–7 turns, so the proposed 6-turn limit is dropped. Preflight/quarantine and failure-specific guidance address independent failure modes; the scoreboard records outcomes but never auto-selects a provider. `ai-reviewer-issue` captures detailed local evidence with one short session summary. Preserve fail-closed safety and never broaden permissions. |
