...
| Touch Kimi, `ai-kimi`, or the Kimi skill | **The STEP 0 VERIFICATION header at the top of `bin/ai-kimi` FIRST**, then `config/kimi/readonly-review.md`, `skills/shared/kimi-code-delegation/SKILL.md`, `tests/test-ai-kimi.sh`, plus `plan_kimi-persistent-implementation-sessions.md` while its STATUS has open rows and `plan_kimi-incomplete-implementation-recovery.md` | Reviews are read-only ONLY because of the agent profile; plain `kimi -p` writes files freely, and its tool names are case-sensitive and fail silently to no-tools-at-all. Verify both that a review can read and that it cannot write. Kimi reports no tokens, cost, or model — never quote figures for a Kimi run |
...
| Spec Kit — what was decided | `docs/github-spec-kit-evaluation.md` §8 (binding) | `specify-cli` declined, 4-phase plan dropped, 2026-08-13 |
...
| deferred | Move root plans into `specs/` | Rest of the Spec Kit plan dropped 2026-08-13 — `docs/github-spec-kit-evaluation.md` §8. Revisit once `plan_context-engineering-consolidation.md` closes |
