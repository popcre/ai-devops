...
- **What it does:** installs CLI helpers (`ai-devops`, `ai-workspace-status`,
  `ai-codex-review`, `ai-model-call`, `ai-run-task`, `ai-glm`, `ai-grok-review`, `ai-grok-implement`, `ai-kimi`) that drive a staged coding
...
| Touch GLM, `ai-glm`, the OpenCode server, or the Windows GLM setup | `AGENTS.md`, **`docs/glm-opencode.md` (read section 5 "Hard-won constraints" FIRST)**, `bin/ai-glm`, `bin/setup-opencode-glm.sh` / `.ps1`, `config/opencode/*`, `skills/shared/ask-glm/SKILL.md`, `tests/test-ai-glm.sh`, `tests/test-windows-scripts.sh` | One-shot implementations have v3 records, full-run repository/caller/name locks, exact abort, and conservative dead-owner recovery. Failed changed work stays nonzero and is exported only as `.incomplete.patch` plus `.incomplete.md`; no-change failures create no patch, export failure alone preserves the exact clone, and nothing is auto-applied. Section 5 lists 32 constraints that each guard a measured failure. Do not "simplify" the job lifecycle, completion rule, remote-less clone, ASCII-only `.ps1` rule, or agent `tools:` maps without re-measuring. Completed implementation records are `plan_glm-implementation-job-tracking.md`, `plan_ai-glm-permission-deadlock.md`, and `plan_ai-glm-permission-failures.md` (CLOSED 2026-08-12). **There is no open GLM plan.** Measured 2026-08-12 (constraint 32): implement mode is never asked for `edit`, `write`, `patch`, or `bash`, so do NOT widen `classify_permissions` on a code reading — the closed plan's standing reopen rule requires a recorded `failure_detail.action` naming a real payload first. A `transport-failed` code is a dead local server, not a permission problem. |
| Touch Grok, `ai-grok-review`, `ai-grok-implement`, or the Grok skill | `AGENTS.md`, `bin/ai-grok-review` (read the STEP 0 VERIFICATION header FIRST), `bin/ai-grok-implement` (read its STEP 0 VERIFICATION header FIRST), `skills/shared/grok-cli/SKILL.md`, `tests/test-ai-grok-review.sh`, `tests/test-ai-grok-implement.sh` | Completion is proven by a terminal `stopReason` in the JSON, NEVER by exit status — exit 0 with a 0-byte file is Grok's normal early-return appearance. Do not remove `--max-turns`, broaden the permission set, add a flag passthrough, or "simplify" the wait loop or the per-repo lock. Each guards a failure that cost real money on 2026-08-05. For implementation runs: `grok --worktree` is ACCEPTED AND SILENTLY IGNORED in headless mode (measured 0.2.112, 2026-08-12) — Grok then edits the primary checkout, which on a detached HEAD serves stale files and produces `NotFound` tool errors. `ai-grok-implement` creates the worktree with git, bases it on `origin/main`, and never passes `--worktree` or `--permission-mode auto`. Do not "restore" either |
| Touch Kimi, `ai-kimi`, or the Kimi skill | `AGENTS.md`, `plan_kimi-persistent-implementation-sessions.md` while its STATUS has open rows, `plan_kimi-incomplete-implementation-recovery.md`, `bin/ai-kimi` (read the STEP 0 VERIFICATION header FIRST), `config/kimi/readonly-review.md`, `skills/shared/kimi-code-delegation/SKILL.md`, `tests/test-ai-kimi.sh` | Reviews are read-only ONLY because of the agent profile — plain `kimi -p` writes files freely (proven). Tool names in that profile are CASE-SENSITIVE and fail SILENTLY to no-tools-at-all: verify BOTH that a review can read and that it cannot write. Completion is the terminal `session.resume_hint` record, never exit status. Failed write runs stay nonzero; changed work is exported only as `.incomplete.patch` plus `.incomplete.md`, never auto-applied. Kimi has no `--max-turns` and reports no tokens/cost/model — never quote figures for a Kimi run |
