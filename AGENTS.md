...
| Reviewer-log repair rounds or durable checkpoints | [`plan_reviewer-log-repair-checkpoints.md`](plan_reviewer-log-repair-checkpoints.md) STATUS, [`docs/reviewer-issues.md`](docs/reviewer-issues.md), `ai-reviewer-issue maintenance show` | Resume the frozen round; account for every candidate; preserve incident evidence and carry unfinished repairs forward before completing |
| Incremental reviewer-log repair rounds or scan checkpoints | [`plan_reviewer-log-repair-checkpoints.md`](plan_reviewer-log-repair-checkpoints.md) STATUS, issue #308, [`docs/reviewer-issues.md`](docs/reviewer-issues.md), `bin/ai-reviewer-issue`, matching tests | Incident resolution is not log coverage; preserve incidents in place and advance a checkpoint only after a fully reconciled, evidence-backed round |
...
| Upgrade Grok Build to 1.0.13 or integrate post-1.0.5 wrapper behavior | [`plan_grok-build-1.0.13-wrapper-integration.md`](plan_grok-build-1.0.13-wrapper-integration.md) STATUS, issue #251, [`docs/grok-build-1.0.13-release-disposition.md`](docs/grok-build-1.0.13-release-disposition.md), `config/provider-cli-versions.json`, current Grok wrapper/installers/tests | Preserve every reviewer boundary; classify every release item and qualify exact 1.0.13 before adapting behavior |
| Change which third-party provider CLI version is supported | `config/provider-cli-versions.json`, `bin/ai-provider-version`, both provider installers and their tests | The pinned version is a repository fact; change the policy file and re-qualify, never loosen the wrappers' refusal |
...
| A reviewer slot held by a worker that died silently, or a lane waiting for a slot with nothing recording it | [`plan_reviewer_lease_liveness.md`](plan_reviewer_lease_liveness.md) STATUS, issue #283, `u2giants/shared-db#2345` | Read `plan_reviewer_lease_capacity_truth.md` STATUS in `u2giants/shared-db` first — it is complete. The code lives only in shared-db; execute it there. Never relax the six terminal failure codes, never release on age alone, and prove any reclaim rule against a lane that is genuinely still working |
