...
| Build or continue the Muse Spark 1.2 OpenCode harness | **[`plan_muse-opencode-harness.md`](plan_muse-opencode-harness.md) STATUS first**, then `docs/glm-opencode.md` section 5, `plan_reviewer-system-repair.md` STATUS/DRIFT, and the files named by the current Muse plan step | Issue #40 owns this work. Keep Muse and GLM as separate live services but one tested safety core; use Meta's standard service tier, never silently switch to the Contributor/data-training tier |
