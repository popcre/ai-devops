...
| Add, implement, change, or test `ai-gemini`, Gemini 3.7 Flash review, or Antigravity reviewer integration | **[`plan_gemini_reviewer_safety_repair.md`](plan_gemini_reviewer_safety_repair.md) STATUS first while issue #38 is open**, then the historical [`plan_ai-gemini-wrapper.md`](plan_ai-gemini-wrapper.md), [`docs/ai-gemini-wrapper-investigation.md`](docs/ai-gemini-wrapper-investigation.md), the `bin/ai-gemini` verification header, shared Gemini skill, and `tests/test-ai-gemini.sh` | The wrapper is implemented but **quarantined**. Offline gates reject file changes, wrong conversation/model, missing reports, interruptions, and stale code, but neither Windows nor Ubuntu has passed the required live hostile qualification. Do not use Gemini as approval, remove quarantine, change global Antigravity settings, or use `--dangerously-skip-permissions`. |
