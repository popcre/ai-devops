...
| Fix delegated-review latency, no-verdict runs, provider rotation, exact-head invalidation, or non-database work consuming shared-db agents | [`fix_reviewer_system.md`](fix_reviewer_system.md), then the affected Grok/GLM/Kimi wrapper instructions above | Preserve fail-closed safety. The repair is sealed review packets, short ordinary budgets, early verdicts, provider preflight, stable-head scheduling, and strict issue routing — not broader permissions or weaker approval gates |
