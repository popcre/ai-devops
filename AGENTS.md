...
| Run, debug, or change a delegated review that starts inside a **linked Git worktree** | **The header of `bin/ai-review-sandbox` FIRST**, then `tests/test-ai-review-sandbox.sh` and the `review_boundary` block in `bin/ai-glm`, `bin/ai-kimi`, `bin/ai-qwen`, `bin/ai-grok-review` | Every delegated reviewer is given exactly ONE directory, and a linked worktree's `.git` is a FILE pointing outside it — pointing a reviewer at the raw worktree kills the run before it reads any code (observed 2026-08-17, GLM). The wrappers hand over a self-contained disposable snapshot instead. Do NOT "simplify" that away, do not point a reviewer at a raw worktree, and do not widen a reviewer's boundary to include the main repo (no reviewer we drive supports a second directory). `ai-codex-review` and `ai-deepseek-agent` send the diff as text and are deliberately NOT wired |
...
| Fix delegated-review latency, no-verdict runs, provider health, failure handling, exact-head invalidation, or non-database work consuming shared-db agents | **[`plan_reviewer-system-repair.md`](plan_reviewer-system-repair.md) FIRST — its STATUS table and newest DRIFT RECORDED block**, then [`fix_reviewer_system.md`](fix_reviewer_system.md), `bin/ai-review-preflight`, `bin/ai-review-scoreboard`, and the affected wrapper instructions above | The evidence packet shipped. Five additional Grok 4.6 reviews returned 5/5 usable verdicts in 6–7 turns, so the proposed 6-turn limit is dropped. Preflight/quarantine and failure-specific guidance address independent failure modes; the scoreboard records outcomes but never auto-selects a provider. Preserve fail-closed safety and never broaden permissions. |
| Build, change, or debug a review evidence packet | **The header of `bin/ai-review-packet` FIRST**, then `tests/test-ai-review-packet.sh` and `prepare_review()` in `bin/ai-grok-review`, `bin/ai-glm`, `bin/ai-kimi` | The packet is **ADDITIVE**: reviewers keep full read and grep over the whole directory and the manifest says so. Do NOT turn it into a sealed room — `reviewer_retains_access_outside_the_packet` guards that. A fact belongs in the packet only if a reviewer could not get it without a shell; policy files are referenced by path, never inlined. Nothing is ever silently truncated |
...
| Touch Kimi, `ai-kimi`, or the Kimi skill | **Read `plan_kimi-windows-execution-reliability.md` STATUS first while issue #31 is open**, then the STEP 0 VERIFICATION header at the top of `bin/ai-kimi`, `config/kimi/readonly-review.md`, `skills/shared/kimi-code-delegation/SKILL.md`, `tests/test-ai-kimi.sh`, `plan_kimi-persistent-implementation-sessions.md`, and `plan_kimi-incomplete-implementation-recovery.md` | Reviews are read-only ONLY because of the agent profile; plain `kimi -p` writes files freely, and its tool names are case-sensitive and fail silently to no-tools-at-all. Verify both that a review can read and that it cannot write. Kimi reports no tokens, cost, or model. Never quote figures for a Kimi run. Until issue #31 is complete, credentialed Windows Kimi execution belongs in the Full Access main task, not a restricted delegated task |
...
| Implement the POP Business Logic Library Skill | [`plan_pop-business-rules-skill.md`](plan_pop-business-rules-skill.md) STATUS first, then `docs/skills-map.md`, `docs/skills-usage-guide.md`, `docs/skill-trigger-eval.md`, and the canonical library map in `u2giants/shared-db/docs/business-rules/application-map.md` | The Skill is a procedure and router only. Never copy business rules into `ai-devops`, the Skill body, or application-specific rulebooks. |
