...
| Implement concurrent-session work claims | [`plan_ai-devops-work-claims.md`](plan_ai-devops-work-claims.md) STATUS, issue #131 | Still blocked. Do not start claim code until `plan_repo-throughput-restructure.md` step 1.2 closes — as of 2026-08-28 its fix is written but its ten-run proof is not done. V1 is task-only and advisory; no orchestrator, claims file, hooks, takeover command, or required check |
| Slow CI, flaky required checks, merge-queue ejections, or "sessions are burning hours and shipping nothing" | [`plan_repo-throughput-restructure.md`](plan_repo-throughput-restructure.md) STATUS | Read its STATUS table first. The frozen-baseline cause of the reviewer flake is **fixed** (`fe7c0606`, progress-sensitive waits, unit-proven by `tests/test-lib-test-timing.sh`) — do not redo it. Step 1.2 is **still not satisfied** because the flake rate is unmeasured: no clean ten-run series exists. Evidence and the two abandoned attempts are under `tests/verification/reviewer-flake-89/`. Never weaken, quarantine, or raise a timeout on a test to make a lane green (Decision B) |
| A reviewer-suite wait that reports a "stall", or choosing between `poll_until` and `poll_until_progress` | [`plan_progress-wait-misuse-guard.md`](plan_progress-wait-misuse-guard.md) STATUS | Read its STATUS table first; start at Step 0. A stall window is only valid if the progress expression is **verified to change while the system is healthily waiting** - a signal that never moves makes the wait give up early and report a stall that looks like a hang in the code under test. That misuse ejected PR #142 from the merge queue (run `33144576111`). A `cancelled` CI job is never a test result: check the duration against `timeout-minutes` first |
| Windows CI jobs, the self-hosted runner, or a local reviewer test series | [`docs/self-hosted-windows-runner.md`](docs/self-hosted-windows-runner.md), [`docs/critical-incidents.md`](docs/critical-incidents.md) 2026-08-28 | `windows-offline` and `windows-reviewer-safety` run on one self-hosted runner on `edge-dev`. **That machine hosts either the CI checks or a local test series, never both** — they share process names, so a local series can cancel a live CI job and a check can inflate a series. Confirm the runner is idle before starting one. This is a public repository: fork-PR approval must stay `all_external_contributors` |
