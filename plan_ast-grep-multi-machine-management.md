...
| 5. Add the same pinned package to the Ubuntu Ansible role | ✅ source complete; apply gated | 2026-08-30 | Ansible commit `a98ec32`, PR #13, and read-only run `33347170807` pass lint, syntax, focused source test, and preview. Preview is exactly three ast-grep changes: `/opt/ast-grep`, isolated pinned npm package, and `/usr/local/bin/ast-grep`; OS `sg` assertions pass. PR remains unmerged pending owner authorization. |
