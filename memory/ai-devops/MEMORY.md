...
- [Grok headless early return](grok-headless-early-return.md) — `grok` exits before it finishes (exit 0 + 0-byte file); use `ai-grok-review`, never trust exit status, and `grok doctor` is not an auth check.
- [Kimi read-only agent profile](kimi-readonly-agent-profile.md) — plain `kimi -p` writes files freely; read-only comes only from --agent-file, whose tool names are case-sensitive and fail silently to no-tools-at-all.
