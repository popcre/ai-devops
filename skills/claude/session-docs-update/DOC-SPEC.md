...
- `AGENTS.md`: only for high-signal ROUTING future AI sessions must see quickly: task routing, identifiers, critical warnings, and pointers to detail docs. **Hard size cap — see the SKILL.md rule "AGENTS.md size cap".** Check `wc -c AGENTS.md` before appending; soft ceiling 60 KB, hard ceiling 80 KB. Content that grows one entry per session does NOT go here: a new quirk goes in `docs/idiosyncrasies.md`, an incident write-up in `docs/incident-log.md`, in-flight status in `docs/pending-work.md` — each leaving only a one-line title index entry in `AGENTS.md`.
