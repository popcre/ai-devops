...
MIGRATION AUTHORS (max 3)     : <agent / branch / claim / lease expiry>
OWNER of preview              : <agent or NONE>
OWNER of merge                : <agent or NONE>
...
**Ownership rules — state the owner in every brief:**
- Up to **three** agents may author unrelated migrations in isolated worktrees.
- Only **one** agent may apply to preview at a time.
- Only **one** pull request may merge at a time.
- Only **one** production promotion may run at a time.
...
> **CURRENT RULE, superseding the historical text below (owner ruling 2026-08-14):**
> use `scripts/manage-migration-author-lanes.mjs --claim` from `shared-db`.
> It permits up to three unrelated migration authors, atomically reserves each
> version, creates the exact object claim and records an expiring owner lease.
> Preview, merges and production promotion remain strictly one at a time. Run
> `--cleanup-stale` so expired ownership does not clutter the board. Never reuse
> a reserved version. Before preview and merge, update from newly merged `main`
> and rerun version/object and existing guards.

### Historical pre-2026-08-14 dispatch workflow, retained only for incident context

Do not execute the commands or allocation advice in this historical subsection.
The current command above and the concise `SKILL.md` procedure replace them.
