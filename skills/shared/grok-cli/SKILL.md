...
Every paid turn runs with a temporary empty user home, a separate isolated
`GROK_HOME` containing only the same-inode authentication link, and a neutral
directory. Grok's native, project, plugin, Claude, Cursor, and standard
`.mcp.json` discovery is excluded. The wrapper
