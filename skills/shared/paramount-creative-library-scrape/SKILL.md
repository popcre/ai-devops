...
6. **Do not CHANGE the shared database from this skill, and do not load rows into it.** Route schema or load work to the active `shared-db` orchestrator by opening a GitHub issue — `gh issue create --repo u2giants/shared-db --label db-work --title "<the outcome you need>" --body-file <file>` — and stop. **Read-only inspection is explicitly allowed and needs no issue:** you may read the live shared schema in full (schemas, tables, columns, keys and relationships, indexes, constraints, views, functions/RPCs, triggers, RLS policies, migration history, generated types, metadata, safe sample data) and compare it against the Paramount source shape to report gaps. That comparison is the *point* of a schema review and is not "database work". Rule 5 still binds while you do it: licensed Paramount rows and examples stay in the approved private source-data repo and never appear in `shared-db`, in this public repo, in a GitHub issue, in logs, in prompts sent to outside services, in commit messages or in PR text — describe the shape in the request, never paste the data.
