...
>    `xjcyeuvzkhtzsheknaiu`, which is not a project in this account. Preview is
>    **`rjyboqwcdzcocqgmsyel`** (Supabase branch `shared-db-schema-rehearsal`);
>    production is `qsllyeztdwjgirsysgai`. Fixed throughout.
...
4. **Preview first**, then production. Link with the matching DB password
   ...
   item for `rjyboqwcdzcocqgmsyel`):
   ```bash
   cat supabase/.temp/project-ref            # PROVE the target (AGENTS.md §4.2)
   supabase link --project-ref rjyboqwcdzcocqgmsyel   # preview ONLY
   ...
   `supabase link --project-ref qsllyeztdwjgirsysgai && supabase db push`. Do NOT
...
- shared backend (prod): `qsllyeztdwjgirsysgai`  ·  preview branch: `rjyboqwcdzcocqgmsyel`
