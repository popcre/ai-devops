...
| 8 | Codex compensating control (no Stop hook exists) | ✅ done 2026-08-26 | The closeout contract is enforced by the Codex global and the shared `skills/shared/wrap-up/SKILL.md`; audit reported 0 new overlaps |
...
| 13 | Close issue #103's future-promise gap | ✅ implemented and verified 2026-09-16 | The Claude hook records per-turn activity through `UserPromptSubmit` and `PostToolUse`, distinguishes inspection from implementation/test actions, and rejects future-tense action promises at `Stop` when no execution action occurred. Unknown connectors default to inspection, and a separate monotonic marker prevents parallel inspection from erasing execution evidence. Both globals carry the same plain-language rule and `context-audit.py` enforces parity. Hook suite: 50 passed; strict context audit passed; the exact Codex regression ran three times after installation with zero false completions. The governed review front door now forwards packet-bound test/source options. Evidence: `tests/verification/completion-honesty/2026-09-16-issue-103.md`. Exact-state review remains the final pre-merge gate. |
