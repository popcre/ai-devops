...
| 5 | Live qualification and landing | ✅ complete | Live-qualified 2026-09-07, then **re-proved 2026-09-16 on EDGE-DEV** after the official standalone install was missing and the prior qualification hashes no longer matched. Fresh `ai-review-preflight qualify qwen` succeeded; `ai-review-preflight status qwen` reported `usable=true` while still `registry_state=registered`. A governed review of a planted inverted `is_even` returned `VERDICT: REJECT f3aa8eb23ee739d4f8c5b5238520791cb65d217a` quoting `return n % 2 == 1`, requested and returned model `qwen3.8-max`. Original 2026-09-07 root causes remain repaired, not waived: (a) Qwen 0.21.15 re-execs `cli-entry -> cli -> cli`, so the old `process.env` Proxy hid the key after the handoff file was deleted; (b) the Coding Plan subscription key is dead (401), so the wrapper uses the Model Studio pay-per-token endpoint and reports that lane in `doctor`. Offline proof at landing: `tests/test-ai-qwen.sh` 101/101. |

Live proof is done. Do not redo this plan. A later wrapper, runtime, or preloader hash change will automatically restore quarantine until `ai-review-preflight qualify qwen` is run again on that host. See [`docs/verification/qwen-rotation-restore-2026-09-16.md`](docs/verification/qwen-rotation-restore-2026-09-16.md).
...
live proof is complete. The STATUS table above is authoritative.
