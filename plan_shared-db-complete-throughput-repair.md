...
| 7A. Remove every numeric concurrency ceiling on sub-agents, runners, and same-reviewer reviews | 🟨 in progress | 2026-09-16 | ai-devops: Kimi, Qwen, Gemini review locks now keyed per review (Grok already was; GLM/Muse/DeepSeek per session; Codex/Claude none); orchestrator skill no longer waits on busy reviewers; fixtures prove 3 same-provider reviews run concurrently with separate session/copy/verdict (gemini 72/0, kimi 244/0, qwen 166/0). Workflow concurrency groups audited: per-PR/ref only, none serialize unrelated work. shared-db reviewer-busy + 100-author test owned by another session in u2giants/shared-db #3130 / PR #3132 (non-orchestrator); author-lane cap already removed via #431; #2773 (non-orchestrator) open for API fan-out. Remaining: live proof (3 simultaneous same-provider reviews on different PRs). |
