...
How leftover live proofs are sized (one unproven step, one issue, one session, filed when the code lands) is owned by [`plan_live-proof-session-sizing.md`](plan_live-proof-session-sizing.md) / [#511](https://github.com/popcre/ai-devops/issues/511). Never save several unproven steps to hand to a later chat.
