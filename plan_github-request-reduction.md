...
| P5. Remove repeated BlockerWatch scans and writes | In progress, 2026-09-25: PR #806 (shared open-issue snapshot) and #812 (PR depends_on) already live. This landing routes remaining REST re-reads (wake checks, digest lookup, parked-work propagation, link ids) through that snapshot; empty-body tab-collapse in the links parser is fixed. Replay 240 issues/60 links identical to previous build. REST-savings replay: previous 3 REST → now 1 REST on wake/digest/parked-work. Before sample (edge-dev call log, 2026-09-25 8:00–9:00 AM EDT): 89 BlockerWatch calls (50 REST, 37 GraphQL). After sample pending one comparable hour after install. Remaining: #809, after sample, then mark done | P1–P3; serialize with P4 helper edits | Scan/write counts plus wake, alarm and dependency-link equivalence |
