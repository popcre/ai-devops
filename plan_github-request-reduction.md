...
| P5. Remove repeated BlockerWatch scans and writes | In progress, 2026-09-24: PR #806 (alarm and link scans share one open-issue read; replay 240 issues/60 links identical to previous build) and PR #812 (a depends_on naming a pull request no longer fails the links scan, which had re-run the full scan every 10 minutes for days) merged and live on edge-dev 9:11 PM EDT. Remaining: #809 (alarm read cache and node budget never work), propagate REST savings, before/after traffic sample | P1–P3; serialize with P4 helper edits | Scan/write counts plus wake, alarm and dependency-link equivalence |
