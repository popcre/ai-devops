...
- **GLM never reviews GLM-orchestrated work (owner ruling 2026-09-17).** Your
  engine is GLM-5.3. Never perform or arrange a GLM review of work a ZCode
  session orchestrated; when a review is needed it goes to a non-GLM provider.
  Reviewing Claude- or Codex-orchestrated work as a delegated second opinion is
  fine when asked through the normal wrapper flows.
...
- **Frontier-plan delegation.** If a non-Flash GLM session has written an
  implementation-ready plan, delegate every independent, bounded implementation
  slice to a lower-cost model. The planner keeps task decomposition, acceptance
  criteria, integration, final review, and every irreversible decision. A slice
  is implementation-ready only when it names its files or interfaces, exact
  success checks, constraints, and what must not change. For ordinary bounded
  implementation, use the latest GLM Flash that the managed ZCode harness proves
  available. If the harness cannot select and prove a Flash model, this
  lower-cost route is unavailable; do not silently substitute a non-Flash model.
...
- Back up configuration before editing it, change existing settings in place,
  avoid duplicate keys, and validate the result. ZCode configuration is
  `~/.zcode/cli/config.json` (user scope) — its MCP schema is STRICT, and one
  unknown key silently drops the whole server; config-file hooks never run
  unless `hooks.enabled` is `true`. Never change Claude or Codex configuration
  as part of ZCode work. Managed edits go through the toolkit's writers
  (`bin/configure-zcode-mcps.ps1`, the hook installer), never hand-edits.
- **Driving ZCode headlessly from another session:** use `ai-zcode ask` — the
  governed wrapper. Never call the zcode.cjs bundle directly; `--prompt`
  defaults to yolo mode and the raw env assembly is easy to get wrong.
