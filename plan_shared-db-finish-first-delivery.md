...
Production Supabase project: `qsllyeztdwjgirsysgai`. Preview branch/project reference: `rjyboqwcdzcocqgmsyel`. These identifiers must be proved immediately before any write; they are never inferred from intent.
...
- For preview identity and ledger, call or import read-only helpers from `scripts/check-migration-ledger-drift.mjs --target preview` after proving ref `rjyboqwcdzcocqgmsyel`. Exit 2 or unreadable state is `GITHUB/ACCESS UNAVAILABLE`, never `CLEAN`. Do not import these readers into the claim mutex path.
...
- exact preview identity `rjyboqwcdzcocqgmsyel`;
...
- Preview ref: `rjyboqwcdzcocqgmsyel`; production ref: `qsllyeztdwjgirsysgai`.
