...
| `failure: local_dependency_unavailable` before a turn | The wrapper's local OpenCode health check failed. No provider request was made, so this must never be recorded as `provider_unavailable`. | Run `ai-glm server start`, then retry. Use `ai-glm doctor` for the full diagnosis. The wrapper does not auto-start a deliberately stopped service. |
...
| A persistent REVIEW session accepts a prompt and returns no answer at all | Observed 2026-08-12 on session `context-engineering-consolidation-review`: three `ask` turns returned no model response, including one after a service restart, while `ai-glm doctor` passed every check. Never diagnosed. It is NOT the transport bug, which fails loudly with `transport-failed`. | Check `ai-glm show <name>` for a `turn-timeout`, and `ai-glm transcript <name>` for a wedged tool. **Do not report a GLM verdict when this happens** - an empty answer is not agreement. If it recurs, capture the record and transcript before deleting the session, because this is the only recorded instance. |
