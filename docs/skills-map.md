...
| ⚙ `kimi-code-delegation` | Delegates scoped coding tasks to Kimi Code CLI in headless mode. Reviews are structurally read-only. Write runs are isolated and one-shot; failed changed work stays nonzero and returns a clearly marked incomplete patch for manual review, never auto-apply. Shared by Claude and Codex/ChatGPT. | "use Kimi" / "delegate this to Kimi" |
