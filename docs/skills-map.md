...
| ⚙ `implementation-plan-writer` | Writes (or judges) an implementation plan a brand-new, lower-cost implementation session can execute perfectly: ultimate goal in plain English first, all background, rejected approaches, locked-vs-open decisions, per-slice ownership + boundaries + checks, per `implementation-plan-standard.md`, with a mandatory self-audit gate. Every new plan gets a matching `HANDOFF.d/` entry, and the two files link to each other. Shared by Claude, Codex/ChatGPT, and ZCode. | "write an implementation plan" / "plan how we'll build X" / "give another session a plan to implement this" |
