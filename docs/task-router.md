...
| The standing rules every helper model is given (Grok, GLM, Gemini, Qwen, DeepSeek, Muse, Kimi) | `bin/ai-helper-global` header, [`../templates/system/AGENTS-global-helper.md`](../templates/system/AGENTS-global-helper.md), `tests/test-ai-helper-global.sh` | One source for all seven providers: change the template, never a wrapper's copy. The prepend is fail-open and goes in DeepSeek's USER message, never its system message |
