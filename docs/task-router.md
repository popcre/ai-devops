...
| **BlockerWatch** — nested blocker chains, a session waiting on another issue, waking a waiting Claude/Codex/ZCode session, or an unowned stalled blocker. (The system's name is BlockerWatch, fixed by owner request 2026-09-18; built in #546, extended by #549/#550 — say "BlockerWatch", not "the system from #549/#550".) | `bin/ai-blocker-watch` header, `config/blocker-watch.json`, `tests/test-ai-blocker-watch.sh` | GitHub "blocked by" links are the record; resume only the registered session; a failed tick exits non-zero. One machine, `propagate_on_host`, posts comments and runs the unowned-blocker alarm — every other machine only wakes its own sessions; the alarm flags open blockers with no assignee, no `owner:` line, no open linked pull request and no activity past `alarm_stall_hours` in a daily digest issue in `alarm_digest_repo` (chains `alarm_depth_priority`+ levels deep first) — stalls are findings, not failures. Closed-issue scanning is one batched `search/issues` query per tick. A harness program missing on the machine parks the wait as `unrunnable` with a visible error. Every agent machine gets the schedule from its installer (Task Scheduler via the Windows installer, user crontab via `install.sh`); ZCode wakes need the one-time `zcode.cjs login` (copy the full URL from the terminal — auto-open truncates at `&`) |
