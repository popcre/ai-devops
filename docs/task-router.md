...
| Nested blocker chains, a session waiting on another issue, or waking a waiting Claude/Codex/ZCode session | `bin/ai-blocker-watch` header, `config/blocker-watch.json`, `tests/test-ai-blocker-watch.sh` | GitHub "blocked by" links are the record; resume only the registered session; a failed tick exits non-zero |
