...
| Bundled leftover live proofs or one chat given a whole programme of unproven steps | [`../plan_live-proof-session-sizing.md`](../plan_live-proof-session-sizing.md) STATUS (landed #515; #511 closed) | One unproven live-behavior outcome per session; do not re-implement; do not add steps onto a live proof owner |
...
| Nested blocker chains, a session waiting on another issue, or waking a waiting Claude/Codex/ZCode session | `bin/ai-blocker-watch` header, `config/blocker-watch.json`, `tests/test-ai-blocker-watch.sh` | GitHub "blocked by" links are the record; resume only the registered session; a failed tick exits non-zero. One machine, `propagate_on_host`, posts comments — every other machine only wakes its own sessions. Closed-issue scanning is one batched `search/issues` query per tick. A harness program missing on the machine parks the wait as `unrunnable` with a visible error. Every agent machine gets the schedule from its installer (Task Scheduler via the Windows installer, user crontab via `install.sh`); ZCode wakes need the one-time `zcode.cjs login` (copy the full URL from the terminal — auto-open truncates at `&`) |
