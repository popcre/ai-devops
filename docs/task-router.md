...
| Reviewer rotation membership, unreachable reviewer, concurrency, or CLI version policy | [`reviewer-rotation-rules.md`](reviewer-rotation-rules.md), `bin/ai-reviewer-membership-drift` | Allocator is the source of truth; fix code, config and docs together |
...
| Grok Build upgrade or wrapper integration | [`../plan_grok-build-1.0.13-wrapper-integration.md`](../plan_grok-build-1.0.13-wrapper-integration.md) STATUS, issue #251, [`grok-build-1.0.13-release-disposition.md`](grok-build-1.0.13-release-disposition.md), version policy | Preserve reviewer boundaries; qualify a new floor or major version before changing the policy |
...
| Trust-boundary code review (parsers, pricing, external files) | [implementation-plan standard](../templates/system/implementation-plan-standard.md) adversarial-cases requirement | The first review request carries the completed adversarial-cases table in the PR body |
| Dead reviewer worker or invisible slot wait | [`../plan_reviewer_lease_liveness.md`](../plan_reviewer_lease_liveness.md) STATUS, issue #283, `popcre/shared-db#2345` | Code lives in shared-db; retain terminal failure codes and never release on age alone |
...
| `ai-muse` native-engine parity or default switch | [`../plan_ai-muse-native-engine-parity.md`](../plan_ai-muse-native-engine-parity.md) STATUS, issue #542 | Read the STATUS table first; keep both engines; fail closed on usage honesty; default is muse-code (2026-09-23); rollback `AI_MUSE_ENGINE=opencode` |
| Reviewer investigation mode | [`../plan_reviewer-investigation-mode-option-b.md`](../plan_reviewer-investigation-mode-option-b.md) STATUS, issue #253 and provider child | Investigation is advisory; formal review stays read-only; do not bundle CLI upgrades; Grok #513 is landed — do not redo; remaining children #254–#257; do not implement #249 here |
...
| Throughput, slow CI, repeated tests/reviews, or interrupted delivery | [`../plan_workflow-efficiency.md`](../plan_workflow-efficiency.md) STATUS, issue #650 | One accepted outcome per session; #159 is completed history, not a queue of unfinished children |
| Shared-db application waits, orchestrator throughput, dispatch, migration batching, or database delivery latency | [`../plan_shared-db-complete-throughput-repair.md`](../plan_shared-db-complete-throughput-repair.md) STATUS, closed issue #401 | Preserve safe concurrent author lanes (no numeric cap; locked decision 1) and every database gate; route new defects to their own issues |
| Bundled leftover live proofs, one chat given a whole programme of unproven steps, or proofs saved for a later chat | [`../plan_live-proof-session-sizing.md`](../plan_live-proof-session-sizing.md) STATUS, issue #511 | One unproven live-behavior outcome per session; never save several unproven steps for later; do not add steps onto a live proof owner |
...
| **BlockerWatch** — nested blocker chains, waiting sessions, wake/restart, or unowned stalled blockers. | `bin/ai-blocker-watch`, `config/blocker-watch.json`, `tests/test-ai-blocker-watch.sh`; current repair owner: [`plan_blockerwatch-reliability-repair.md`](../plan_blockerwatch-reliability-repair.md) STATUS; predecessor: [`plan_blockerwatch-parked-work.md`](../plan_blockerwatch-parked-work.md) STATUS | Registration is the DEFAULT for any wait over ~10 minutes, subagents included; `has-wait` is what the closeout hook checks. Until the repair STATUS is complete, verify registration, state-home ownership, harness readiness, scheduler health, and GitHub/local reconciliation rather than trusting task exit 0. The fixed product name is BlockerWatch. |
| A shared-db reviewer that was drawn but never started, or the machine task that reroutes it | `bin/ai-reviewer-start-watch` header, `config/reviewer-start-watch.json`, `tests/test-ai-reviewer-start-watch.sh` | Runs only on `run_on_host` (the machine with the reviewer wrappers: drawing a replacement needs `ai-review-preflight`); each tick refreshes its own shared-db clone to `main`, reads `DRAWN_SINCE` from that repo's workflow and runs the watcher with `--apply`; a failed pass exits non-zero and is logged in `~/.ai-devops/reviewer-start-watch/tick.log` |
| Windows CI, runner, or local reviewer series | [`self-hosted-windows-runner.md`](self-hosted-windows-runner.md), [`independent-windows-runner-setup.md`](independent-windows-runner-setup.md), [`critical-incidents.md`](critical-incidents.md) 2026-09-02 | Use `bin/ai-test-local --check-collision`; block only the same physical host or shared installed runtime, never independent hosted or self-hosted capacity |
| Windows runner maintenance elevation | [`../plan_windows-runner-maintenance-elevation.md`](../plan_windows-runner-maintenance-elevation.md) STATUS, issue #262, [`independent-windows-runner-setup.md`](independent-windows-runner-setup.md) narrow refresh section | One fixed operation only; protected installation and each live host proof remain separate gated outcomes |
...
| C: is full, disk is low, or AI debris is eating the drive | [`disk-space.md`](disk-space.md), then [`critical-incidents.md`](critical-incidents.md) 2026-09-23; prevention: [issue #711](https://github.com/popcre/ai-devops/issues/711) | Never delete dirty working copies or live repos; speed stays on the SSD, bulk on the HDD; daily sweep is backup only |
