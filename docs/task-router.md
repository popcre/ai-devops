...
| Nested blocker chains, a session waiting on another issue, waking a waiting Claude/Codex/ZCode session, or an unowned stalled blocker | `bin/ai-blocker-watch` header, `config/blocker-watch.json`, `tests/test-ai-blocker-watch.sh` | GitHub "blocked by" links are the record; resume only the registered session; a failed tick exits non-zero; the alarm flags open blockers with no assignee, no `owner:` line, no open linked pull request and no activity past `alarm_stall_hours` in a daily digest issue in `alarm_digest_repo` — stalls are findings, not failures |
