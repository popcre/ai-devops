...
- **Qwen Code CLI** — the skill and `ai-qwen` wrapper are repo-owned. Install the
  ...
  string, or `qwen` resolving from npm, is not enough. If Windows already has an
  npm `qwen` on PATH, force the standalone with
