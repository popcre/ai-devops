...
| **Muse reviews** (`ai-muse`) | One protected, read-only Muse Spark 1.2 Contributor review in a disposable self-contained copy | ✅ repo-owned direct runner, pinned OpenCode, evidence packet, `ai-muse doctor` | Meta key is read from the `vibe_coding` 1Password item at review time; no long-running Muse service exists |
