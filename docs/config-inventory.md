...
| **Railway CLI + hosted MCP** | Manages Railway projects from the terminal and gives Claude/Codex OAuth-based Railway tools at `https://mcp.railway.com` | ✅ Windows bootstrap installs `@railway/cli`; `bin/setup-machine.ps1` adds Railway to the shared MCP set and gives Codex its native connection | No repo secret — CLI and MCP use Railway's interactive login/OAuth |
