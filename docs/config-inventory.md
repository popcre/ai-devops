...
| **`~/.codex/config.toml`** | Codex prefs + machine-specific runtime paths + MCP server list | ✅ new configs seed from `config/codex-portable.toml`; established files are never overwritten. The **`1password` MCP block** is managed by `bin/configure-codex-1password.ps1` through `mcp-launch.cmd`; **`chrome-devtools`** is managed independently by `bin/configure-codex-chrome-devtools.ps1` | Machine-specific paths/plugins remain local; managed MCP blocks are token-free |
