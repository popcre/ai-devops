...
| **StepFun Step 5** (`ai-stepfun`, StepCode CLI `step`) | Ubuntu/Linux-only reviewer and implementer used by the shared `stepfun` skill | ✅ repo-owned wrapper, skill, and offline suite. Reviews and `ask` use only read/grep/find/ls with strict approval in a disposable copy; `implement` gets read/bash/edit/write with auto approval in a new worktree and never commits. Refuses to run on Windows (StepCode has no Windows build yet). Live review and implementation proven on 2026-09-25 | Key is the vibe_coding item `stepfun step5 ai api key`, copied once by `ai-stepfun store-key` into the owner-only `~/.config/ai-devops/secrets/stepfun-api-key`; endpoint `https://api.stepfun.ai/v1`, model `step/step-5-preview` |
