...
| **StepFun Step 5** (`ai-stepfun`, StepCode CLI `step`) | Ubuntu/Linux-only reviewer and implementer used by the shared `stepfun` skill | ✅ repo-owned wrapper, skill, and offline suite. Reviews and `ask` use only read/grep/find/ls with strict approval in a disposable copy; `implement` gets read/bash/edit/write with auto approval in a new remote-less clone; a run that commits or adds a remote is refused. Every turn runs under bubblewrap with an empty home and /tmp, so no SSH, git, gh, or 1Password secrets are visible; reviews use the shared sealed evidence packet. Refuses to run on Windows (StepCode has no Windows build yet) or without bubblewrap. Live review and implementation proven on 2026-09-25 | Key is the vibe_coding item `stepfun step5 ai api key`, copied once by `ai-stepfun store-key` into the owner-only `~/.config/ai-devops/secrets/stepfun-api-key`; endpoint `https://api.stepfun.ai/v1`, model `step/step-5-preview` |
...
- **Qwen Code CLI** — the skill and `ai-qwen` wrapper are repo-owned. Install the
  ...
  1Password at provider-call time; on EDGE-DEV that is the Model Studio
  pay-per-token lane, not the dead Coding Plan subscription. Prove the full path
  ...
  `bin/install-windows-ai-provider-clis.ps1 -SelectedProvider qwen`; an npm
  `qwen` on PATH no longer hides it.
