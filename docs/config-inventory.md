...
| **`~/.codex/config.toml`** | Codex prefs + machine-specific runtime paths + MCP server list | ✅ new configs seed from `config/codex-portable.toml`; established files are never overwritten. The **`1password` MCP block** is managed by `bin/configure-codex-1password.ps1` through `mcp-launch.cmd` | Machine-specific paths/plugins remain local; managed MCP blocks are token-free |
...
Open reconciliation plan: [`plan_sync-machine-wrapper-reconciliation.md`](../plan_sync-machine-wrapper-reconciliation.md). Read its STATUS table before changing how “sync my dotfiles” checks local Grok, Kimi, or GLM commands.
